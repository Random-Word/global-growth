...
time.sleep(0.5)
n_fert = fetch_owid('nitrogen-fertilizer-application-per-hectare-of-cropland', 'N fertilizer/ha')
time.sleep(0.5)
...
time.sleep(0.5)
water_stress = fetch_owid('freshwater-withdrawals-as-a-share-of-internal-resources', 'Water stress')
...
KEY_COUNTRIES = [
    ...
    'South Korea', 'Australia', 'Canada', 'Sweden', 'Poland',
]

COUNTRY_COLORS = {
    'United States': '#1f77b4', 'United Kingdom': '#2ca02c', 'Germany': '#9467bd',
    'France': '#8c564b', 'Japan': '#e377c2', 'China': '#d62728',
    'India': '#ff7f0e', 'Brazil': '#17becf', 'Indonesia': '#bcbd22',
    'Nigeria': '#7f7f7f', 'South Korea': '#aec7e8', 'Australia': '#ffbb78',
    'Canada': '#98df8a', 'Sweden': '#ff9896', 'Poland': '#c5b0d5',
    'World': 'black',
}
...
if len(dmc_pc) > 0:
    for name in KEY_COUNTRIES + ['World']:
        ...
        if len(d) > 3:
            ...
            ax.plot(d['Year'], d[dmc_col], label=name,
                   color=COUNTRY_COLORS.get(name, None), linewidth=lw, linestyle=ls)
    ax.set_title('Domestic Material Consumption Per Capita', fontsize=12, fontweight='bold')
...
if len(dmc_pc) > 0:
    ...
    for ent in dmc_pc['Entity'].unique():
        ...
        if len(early) > 0 and len(late) > 0:
            ...
            if e_val > 0:
                ...
                changes.append({'country': ent, 'early': e_val, 'late': l_val,
    ...
    if len(cdf) > 0:
        ...
        ax.hist(cdf['pct_change'], bins=40, color='steelblue', alpha=0.7, edgecolor='white')
        ax.axvline(x=0, color='red', linewidth=2, linestyle='--')
        ax.set_title(f'Change in DMC/Capita (~2000 to latest)\n'
        ...
        for name in ['United States', 'China', 'India', 'Germany', 'United Kingdom', 'Japan', 'Brazil']:
            row = cdf[cdf['country'] == name]
            if len(row) > 0:
                val = row.iloc[0]['pct_change']
                ax.annotate(name.replace('United States', 'US').replace('United Kingdom', 'UK'),
                           xy=(val, 0), fontsize=7, rotation=90, va='bottom', ha='center')

        print(f"\n  Countries with DECLINING DMC/cap: {len(declining)} ({len(declining)/len(cdf)*100:.0f}%)")
        print(f"  Countries with RISING DMC/cap: {len(rising)} ({len(rising)/len(cdf)*100:.0f}%)")
        print(f"  Median change: {cdf['pct_change'].median():+.0f}%")

        # Print key country details
        print(f"\n  Key country DMC/cap changes (2000→latest):")
        for name in ['United States', 'United Kingdom', 'Germany', 'Japan', 'China', 'India',
                      'Brazil', 'Australia', 'South Korea', 'Sweden', 'Nigeria', 'Indonesia']:
            row = cdf[cdf['country'] == name]
            if len(row) > 0:
                r = row.iloc[0]
                print(f"    {name:20s}: {r['early']:.1f} → {r['late']:.1f}t ({r['pct_change']:+.0f}%)")
...
if len(dmc_pc) > 0:
    ...
    latest_dmc = dmc_pc[dmc_pc['Year'] >= dmc_pc['Year'].max() - 2].groupby('Entity')[dmc_col].mean()
    ...
    if len(common) > 10:
        ...
        ax.set_xscale('log')
        ax.set_title('Material Consumption vs Income (latest)', fontsize=12, fontweight='bold')
        ...
        for name in KEY_COUNTRIES:
            ...
            if len(row) > 0:
                ...
                ax.scatter(row['gdp_pc'], row['dmc_pc'], s=60, zorder=5,
        ...
        if len(valid) > 10:
            ...
            x_line = np.logspace(np.log10(valid['gdp_pc'].min()),
                                np.log10(valid['gdp_pc'].max()), 100)
            y_line = 10**(slope * np.log10(x_line) + intercept)
            ax.plot(x_line, y_line, 'r--', alpha=0.5)
            ax.annotate(f'Elasticity: {slope:.2f}\nR²={r**2:.2f}',
                       xy=(0.05, 0.95), xycoords='axes fraction', fontsize=10, va='top')
            print(f"\n  Material-income elasticity: {slope:.2f} (R²={r**2:.2f})")
            if slope < 1:
                print(f"  → Elasticity < 1: richer countries use proportionally LESS material/$ of GDP")
            else:
                print(f"  → Elasticity ≥ 1: richer countries use proportionally MORE material/$ of GDP")

# 33d: Rich vs developing country DMC trends (indexed)
ax = axes[1, 1]
if len(dmc_pc) > 0:
    groups = {
        'Rich avg\n(US, UK, DE, FR, JP)': ['United States', 'United Kingdom', 'Germany', 'France', 'Japan'],
        'China': ['China'],
        'India': ['India'],
        ...
        if len(group_data) > 0:
            ...
            if len(yearly) > 3:
                ...
                if base_val > 0:
                    ...
                    ax.plot(indexed.index, indexed.values, label=label, color=color,
                           linewidth=2 if 'China' in label else 1.5)

    ax.axhline(y=100, color='gray', linewidth=0.5, linestyle=':')
    ax.set_title('Material Consumption Trajectories (indexed)', fontsize=12, fontweight='bold')
    ax.set_ylabel(f'Index (earliest year = 100)')
    ax.legend(fontsize=9)

plt.suptitle('Chart 33: Material Consumption — Country-Level Variation',
...
if len(n_fert) > 0:
    for name in KEY_COUNTRIES + ['World']:
        ...
        if len(d) > 5:
            lw = 2.5 if name == 'World' else 1.3
            ls = '--' if name == 'World' else '-'
            ax.plot(d['Year'], d[n_col], label=name,
                   color=COUNTRY_COLORS.get(name, None), linewidth=lw, linestyle=ls)
    ax.set_title('Nitrogen Fertilizer per Hectare of Cropland', fontsize=12, fontweight='bold')
...
if len(n_fert) > 0:
    ...
    for ent in n_fert['Entity'].unique():
        d = n_fert[(n_fert['Entity'] == ent) & (n_fert['Year'] >= 1970)].sort_values('Year')
        if len(d) < 10:
            continue
        ...
        if peak_val > 10:  # Ignore tiny users
            ...
            peak_data.append({
                'country': ent, 'peak_year': peak_yr, 'peak_val': peak_val,
                'latest_val': latest_val, 'latest_yr': latest_yr,
    ...
    real = n_fert[n_fert['Code'].notna() & (n_fert['Code'].str.len() == 3)]['Entity'].unique()
    ...
    if len(pdf) > 0:
        ...
        if len(peaked) > 0:
            ax.hist(peaked['peak_year'], bins=range(1970, 2025, 5), alpha=0.7,
                   color='steelblue', edgecolor='white', label=f'Peaked ({len(peaked)})')
        ax.axvline(x=2020, color='red', linewidth=1.5, linestyle='--', alpha=0.5)
        ax.set_title(f'When Did N Fertilizer/ha Peak?\n'
        ...
        for name in ['China', 'India', 'United States', 'Germany', 'France', 'Brazil',
                      'United Kingdom', 'Japan', 'Indonesia', 'Nigeria']:
            row = pdf[pdf['country'] == name]
            if len(row) > 0:
                r = row.iloc[0]
                status = 'PEAKED' if r['peaked'] else 'still rising'
                print(f"    {name:20s}: peak {r['peak_val']:.0f} kg/ha ({int(r['peak_year'])}), "
...
if len(n_fert) > 0:
    ...
    if len(common) > 10:
        ...
        ax.set_xscale('log')
        ax.set_title('N Fertilizer/ha vs Income (latest)', fontsize=12, fontweight='bold')
        ax.set_xlabel('GDP per capita (USD)')
        ax.set_ylabel('kg N / hectare')
        ...
        for name in KEY_COUNTRIES:
            ...
            if len(row) > 0:
                ax.annotate(name.replace('United States', 'US').replace('United Kingdom', 'UK')
                           ...
                           fontsize=7, alpha=0.8)
                ax.scatter(row['gdp_pc'], row['n_ha'], s=60, zorder=5,
                          color=COUNTRY_COLORS.get(name, 'red'))
...
if len(n_fert) > 0:
    ...
    for name in peaked_countries:
        d = n_fert[(n_fert['Entity'] == name) & (n_fert['Year'] >= 1980)].sort_values('Year')
        if len(d) > 3:
            ax.plot(d['Year'], d[n_col], label=name, linewidth=1.3,
                   color=COUNTRY_COLORS.get(name, None), linestyle='-')
    ...
    ax.set_title('N/ha: Peaked Countries (solid) vs Still Rising (dashed)',
                fontsize=12, fontweight='bold')
    ax.set_ylabel('kg N / hectare')
    ax.legend(fontsize=8, ncol=2)

plt.suptitle('Chart 34: Nitrogen Fertilizer — Country-Level Divergence',
             fontsize=15, fontweight='bold', y=1.01)
plt.tight_layout()
...
ax = axes[0, 0]
if len(red_list) > 0:
    for name in ['World', 'United States', 'China', 'India', 'Brazil', 'Indonesia',
                  'Australia', 'United Kingdom', 'Germany', 'Japan', 'Nigeria', 'South Africa']:
        d = red_list[red_list['Entity'] == name].sort_values('Year')
        if len(d) > 2:
            lw = 2.5 if name == 'World' else 1.3
            ls = '--' if name == 'World' else '-'
            ax.plot(d['Year'], d[rl_col], label=name, linewidth=lw, linestyle=ls,
                   color=COUNTRY_COLORS.get(name, None))
    ax.set_title('Red List Index by Country (1=safe, 0=all extinct)', fontsize=12, fontweight='bold')
...
if len(red_list) > 0:
    ...
    for ent in red_list['Entity'].unique():
        ...
        if first_val > 0:
            ...
            rl_changes.append({'country': ent, 'first': first_val, 'last': last_val,
                              'change_pct': pct, 'abs_change': last_val - first_val})

    rdf = pd.DataFrame(rl_changes)
    real = red_list[red_list['Code'].notna() & (red_list['Code'].str.len() == 3)]['Entity'].unique()
    ...
    if len(rdf) > 0:
        ...
        ax.hist(rdf['abs_change'], bins=50, color='darkred', alpha=0.7, edgecolor='white')
        ax.axvline(x=0, color='green', linewidth=2, linestyle='--')
        ax.set_title(f'Change in Red List Index (1993→2024)\n'
        ...
        print(f"    Worsening: {len(worsening)} countries ({len(worsening)/len(rdf)*100:.0f}%)")

        # Worst 10
        worst = rdf.nsmallest(10, 'abs_change')
        print(f"\n    Worst 10 RLI declines:")
        for _, r in worst.iterrows():
            print(f"      {r['country']:25s}: {r['first']:.3f} → {r['last']:.3f} ({r['abs_change']:+.3f})")
...
if len(tree_loss) > 0:
    ...
    if len(top10) > 0:
        colors_tc = ['#d62728' if c in ['Brazil', 'Indonesia'] else '#ff7f0e'
                    if c in ['Russia', 'Canada'] else '#2ca02c' for c in top10.index]
        bars = ax.barh(range(len(top10)), top10.values / 1e6, color='darkgreen', alpha=0.7)
        ax.set_yticks(range(len(top10)))
        ax.set_yticklabels(top10.index, fontsize=9)
        ax.set_title(f'Total Tree Cover Loss (2001–{tree_loss["Year"].max():.0f})',
                    fontsize=12, fontweight='bold')
...
if len(forest_area) > 0:
    for name in ['China', 'India', 'Brazil', 'Indonesia', 'United States',
                  'Russia', 'Vietnam', 'United Kingdom', 'France']:
        ...
        if len(d) > 2:
            ...
            if base > 0:
                ...
                ax.plot(d['Year'], indexed, label=name, linewidth=1.5,
                       color=COUNTRY_COLORS.get(name, None))

    ax.axhline(y=100, color='gray', linewidth=0.5, linestyle=':')
    ax.set_title('Forest Area Trends (indexed to earliest year = 100)',
                fontsize=12, fontweight='bold')
    ...
    print(f"\n  Forest area changes:")
    for name in ['China', 'India', 'Brazil', 'Indonesia', 'United States', 'Vietnam',
        ...
        if len(d) > 1:
            ...
            print(f"    {name:20s}: {first[fa_col]/1e6:.1f} → {last[fa_col]/1e6:.1f} Mha "
                  f"({pct:+.1f}%, {first['Year']:.0f}→{last['Year']:.0f})")

plt.suptitle('Chart 35: Biodiversity & Deforestation — Country Decomposition',
             fontsize=15, fontweight='bold', y=1.01)
plt.tight_layout()
...
scorecard_countries = ['United States', 'United Kingdom', 'Germany', 'France', 'Japan',
                       'China', 'India', 'Brazil', 'Indonesia', 'South Korea',
                       'Australia', 'Canada', 'Sweden', 'Poland', 'Nigeria']
...
for country in scorecard_countries:
    ...
    early_co2 = co2_d[(co2_d['year'] >= 2000) & (co2_d['year'] <= 2005)]['co2_per_gdp'].mean()
    ...
    # Forest area change
    if len(forest_area) > 0:
        _, e, l = compute_change(forest_area, 'Entity', 'Year', fa_col, country, start=1990)
        if e > 0:
...
ax.set_title('Ecological Decoupling Scorecard (~2000→latest)\nGreen = improving, Red = worsening',
            fontsize=12, fontweight='bold')
...
if len(valid) > 3:
    ...
    for country in valid.index:
        ax.annotate(country.replace('United States', 'US').replace('United Kingdom', 'UK')
                   .replace('South Korea', 'S.Korea'),
    ...
    # Quadrant labels
    ax.annotate('Carbon ✓ Material ✓\n(full decoupling)', xy=(0.02, 0.02),
               xycoords='axes fraction', fontsize=9, color='green', fontweight='bold')
    ax.annotate('Carbon ✓ Material ✗\n(carbon only)', xy=(0.02, 0.95),
               xycoords='axes fraction', fontsize=9, color='orange', fontweight='bold', va='top')
    ax.annotate('Carbon ✗ Material ✓\n(unusual)', xy=(0.72, 0.02),
               xycoords='axes fraction', fontsize=9, color='orange', fontweight='bold')
    ax.annotate('Carbon ✗ Material ✗\n(no decoupling)', xy=(0.72, 0.95),
               xycoords='axes fraction', fontsize=9, color='red', fontweight='bold', va='top')

    ax.set_xlabel('CO₂/GDP change (%); negative = decoupling', fontsize=11)
    ax.set_ylabel('Material consumption/cap change (%); negative = decoupling', fontsize=11)
    ax.set_title('CO₂ Decoupling vs Material Decoupling\n(~2000 to latest)', fontsize=12, fontweight='bold')
...
plt.suptitle('Chart 36: Multi-Dimensional Decoupling Scorecard',
...
print(f"  {'Country':<20s} {'CO₂/GDP':>8s} {'DMC/cap':>8s} {'N fert/ha':>10s} {'RLI':>8s} {'Forest':>8s}")
print("  " + "─"*70)
for country in scorecard_countries:
    r = sdf.loc[country]

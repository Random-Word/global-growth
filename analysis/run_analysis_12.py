...
print(
    f"  {'Country':<20s} {'Year':>4s} {'Mean':>10s} {'ε=0.5':>10s} {'ε=1':>10s} {'ε=2':>10s} {'Rawls':>10s}"
)
for _, r in latest.sort_values("mean_inc", ascending=False).iterrows():
    print(
        f"  {NAMES.get(r['cc'],r['cc']):<20s} {int(r['year']):4d} ${r['mean_inc']/1000:>8.1f}k ${r['edei_05']/1000:>8.1f}k ${r['edei_10']/1000:>8.1f}k ${r['edei_20']/1000:>8.1f}k ${r['rawls']/1000:>8.1f}k"
    )
...
ax.fill_between(
    ...
    alpha=0.08,
    color="green",
    label="Pro-poor growth (above line)",
)
ax.fill_between(
    [0, lim_max],
    [0, 0],
...
ax.legend(
    handles=legend2 + ax.get_legend().legend_handles, loc="upper left", fontsize=9
)
...
print(
    f"  {'Country':<20s} {'Span':<12s} {'Mean':>8s} {'e=0.5':>8s} {'e=1':>8s} {'e=2':>8s} {'Rawls':>8s} {'Pro-poor?':>10s}"
)
for cc in ALL_CC:
    cdata = df[df["cc"] == cc].sort_values("year")
    if len(cdata) < 2:
    ...
    propoor = (
        "PRO-POOR" if ratio > 1.05 else ("NEUTRAL" if ratio > 0.95 else "PRO-RICH")
    )
    ...
    print(
        f"  {name:<20s} {span:<12s} {fmt(vals['Mean'])} {fmt(vals['e=0.5'])} {fmt(vals['e=1'])} {fmt(vals['e=2'])} {fmt(vals['Rawls'])} {propoor:>10s}"
    )
...
for i, (col, qlabel, qcolor) in enumerate(zip(q_cols, q_labels, q_colors)):
    ax.barh(
        y_pos + i * bar_height - 0.3,
        plot_latest[col] / 1000,
        height=bar_height,
        label=qlabel,
        color=qcolor,
        alpha=0.85,
...
ax.set_title(
    "The Full Income Spectrum: What Each Quintile Actually Earns\n(Latest year, constant PPP $)",
    fontsize=14,
    fontweight="bold",
)
...
for cc, ranks in rank_data.items():
    ...
    if cc in highlight or abs(rank_vals[0] - rank_vals[-1]) > 2:
        ax2.text(
            ...
            va="center",
            color=color,
            fontweight="bold" if cc in highlight else "normal",
        )
...
ax2.set_title(
    "Rank Shifts: Who Rises When\nYou Care About Inequality?",
    fontsize=13,
    fontweight="bold",
)
ax2.invert_yaxis()
...
ax1.set_title(
    "Rich Countries: Welfare-Weighted Income\n(Geometric mean, ε=1)",
    fontsize=13,
    fontweight="bold",
)
...
ax2.set_title(
    "Developing Countries: Welfare Growth\n(Indexed, ε=1)",
    fontsize=13,
    fontweight="bold",
)
...
for ax, cc, color, title in zip(axes.flat, focus_countries, focus_colors, focus_titles):
    ...
    for metric, mlabel, ls in [
        ...
        ax.plot(
            cdata["year"],
            cdata[metric] / base * 100,
            ls,
            color=color,
            linewidth=2 if "Mean" in mlabel else 1.5,
            label=mlabel,
        )

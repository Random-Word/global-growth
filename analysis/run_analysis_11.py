...
rich_data = None
for col in quintile_cols + ['gini', 'gdppc_ppp', 'life_expectancy', 'under5_mortality',
                             'tertiary_enrollment']:
    if col in datasets:
        ...
        if rich_data is None:
            ...
            rich_data = rich_data.merge(df, on=['country_code', 'country', 'year'], how='outer')

if rich_data is not None:
    # Compute real income at each quintile: avg_income_quintile = GDP_pc * (share/20)
    for q, col in [('Q1_bottom20', 'bottom20_share'), ('Q2', 'q2_share'),
                     ('Q3_median', 'q3_share'), ('Q4', 'q4_share'), ('Q5_top20', 'top20_share')]:
        rich_data[f'income_{q}'] = rich_data['gdppc_ppp'] * (rich_data[col] / 20)
    
    print(f"  Rich-country dataset: {len(rich_data)} rows, {rich_data['country_code'].nunique()} countries")
...
country_order = [('USA', 'United States'), ('GBR', 'United Kingdom'), ('FRA', 'France'),
                 ('DEU', 'Germany'), ('JPN', 'Japan'), ('SWE', 'Sweden')]
colors = {'Q1_bottom20': '#d62728', 'Q2': '#ff7f0e', 'Q3_median': '#2ca02c',
          'Q4': '#1f77b4', 'Q5_top20': '#9467bd'}
labels = {'Q1_bottom20': 'Bottom 20%', 'Q2': '2nd quintile', 'Q3_median': 'Middle 20%',
          'Q4': '4th quintile', 'Q5_top20': 'Top 20%'}
...
for idx, (cc, name) in enumerate(country_order):
    ...
    for q in ['Q1_bottom20', 'Q2', 'Q3_median', 'Q4', 'Q5_top20']:
        ...
        if len(series) > 2:
            ax.plot(series['year'], series[col] / 1000, color=colors[q],
                    label=labels[q], linewidth=2 if q in ('Q1_bottom20', 'Q5_top20') else 1.2)
    ...
    if len(b20) > 5:
        first = b20.iloc[0]
        last = b20.iloc[-1]
        pct_change = (last['income_Q1_bottom20'] / first['income_Q1_bottom20'] - 1) * 100
        ax.annotate(f'Bottom 20%: +{pct_change:.0f}%\n({int(first["year"])}→{int(last["year"])})',
...
plt.suptitle('Chart 47: Real Income by Quintile — Everyone Got Richer in Absolute Terms',
...
for cc, name in country_order:
    ...
    print(f"  {name:20s}: Bottom20 ${first['income_Q1_bottom20']:,.0f}→${last['income_Q1_bottom20']:,.0f} "
...
for idx, (cc, name) in enumerate(country_order):
    ...
    if len(d_clean) > 2:
        ...
        for i, (col, color, label) in enumerate(zip(quintile_cols, q_colors, q_labels)):
            ...
            ax.fill_between(years, bottom, bottom + vals, color=color, alpha=0.7, label=label)
            bottom = bottom + vals
        ...
        ax.annotate(f'Top 20%: {first_top:.1f}→{last_top:.1f}% ({direction_top}{last_top-first_top:.1f}pp)\n'
...
plt.suptitle('Chart 48: Income Share Shifts — The Rich Got a Bigger Slice, But the Pie Grew',
             fontsize=15, fontweight='bold', y=1.01)
plt.tight_layout()
...
if 'gdppc_ppp_global' in datasets:
    ...
    comparators = {
        ...
        'THA': 'Thailand', 'IDN': 'Indonesia', 'ZAF': 'South Africa',
        'NGA': 'Nigeria', 'VNM': 'Vietnam', 'BGD': 'Bangladesh',
        'COL': 'Colombia', 'TUR': 'Türkiye',
    }
    ...
    for cc, name in country_order:
        ...
        if len(d) > 0:
            last = d.iloc[-1]
            rich_b20.append({'country': name, 'income': last['income_Q1_bottom20'],
                            'label': f"{name}\nbottom 20%", 'color': '#d62728'})
    ...
    for cc, name in comparators.items():
        ...
        if len(d) > 0:
            last = d.iloc[-1]
            dev_avg.append({'country': name, 'income': last['gdppc_ppp_global'],
                            'label': f"{name}\naverage", 'color': '#1f77b4'})
    ...
    ax.set_title('Bottom 20% in Rich Countries vs Average in Developing',
    ...
    legend_elements = [Patch(facecolor='#d62728', alpha=0.8, label='Rich-country bottom 20%'),
                       Patch(facecolor='#1f77b4', alpha=0.8, label='Developing-country average')]
    ax.legend(handles=legend_elements, fontsize=9, loc='lower right')

# 49b: Gini trend comparison
ax = axes[1]
gini_colors = {'USA': '#1f77b4', 'GBR': '#ff7f0e', 'FRA': '#2ca02c',
               'DEU': '#d62728', 'JPN': '#9467bd', 'SWE': '#8c564b'}
for cc, name in country_order:
    d = rich_data[rich_data['country_code'] == cc].sort_values('year')
    ...
    if len(d) > 2:
        ...
        # Annotate latest
        last = d.iloc[-1]
        ax.annotate(f'{last["gini"]:.1f}', xy=(last['year'], last['gini']),
                    fontsize=9, fontweight='bold', color=gini_colors[cc])

ax.set_title('Gini Coefficient Trends — Inequality Trajectories Diverge',
             fontsize=12, fontweight='bold')
ax.set_ylabel('Gini coefficient')
ax.legend(fontsize=9)
ax.annotate('Higher = more unequal\nUS stands alone',
            xy=(0.02, 0.95), xycoords='axes fraction', fontsize=9, va='top',
            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))

plt.suptitle('Chart 49: Rich-Country Poor vs the World — Context Matters',
             fontsize=15, fontweight='bold', y=1.01)
plt.tight_layout()
...
ax = axes[0, 0]
le_colors = {'USA': '#1f77b4', 'GBR': '#ff7f0e', 'FRA': '#2ca02c',
             'DEU': '#d62728', 'JPN': '#9467bd', 'SWE': '#8c564b'}
for cc, name in country_order:
    d = rich_data[rich_data['country_code'] == cc].sort_values('year')
    ...
    d = d[d['year'] >= 1970]
    if len(d) > 2:
        ax.plot(d['year'], d['life_expectancy'], label=name, color=le_colors[cc], linewidth=1.5)
ax.set_title('Life Expectancy at Birth', fontsize=12, fontweight='bold')
ax.set_ylabel('Years')
ax.legend(fontsize=8)
ax.annotate('US diverges from peers\nafter ~2010 (opioids, COVID)',
            xy=(2010, 76), fontsize=9,
            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
...
for cc, name in country_order:
    ...
    d = d[d['year'] >= 1970]
    if len(d) > 2:
        ax.plot(d['year'], d['under5_mortality'], label=name, color=le_colors[cc], linewidth=1.5)
ax.set_title('Under-5 Mortality (per 1,000 live births)', fontsize=12, fontweight='bold')
...
for cc, name in country_order:
    ...
    d = d[d['year'] >= 1970]
    if len(d) > 2:
        ax.plot(d['year'], d['tertiary_enrollment'], label=name, color=le_colors[cc], linewidth=1.5)
...
rows = []
for cc, name in [('USA', 'US'), ('GBR', 'UK'), ('FRA', 'France'),
                  ('DEU', 'Germany'), ('JPN', 'Japan'), ('SWE', 'Sweden')]:
    d = rich_data[rich_data['country_code'] == cc].sort_values('year')
    b20 = d.dropna(subset=['income_Q1_bottom20'])
    ...
    if len(b20) > 2 and len(le) > 2:
        first_b20 = b20[b20['year'] >= 1985].iloc[0] if len(b20[b20['year'] >= 1985]) > 0 else b20.iloc[0]
        last_b20 = b20.iloc[-1]
...
if rows:
    ...
    ax.set_title('Summary: Bottom 20% Got Richer Everywhere', fontsize=12, fontweight='bold')

plt.suptitle('Chart 50: Human Development in Rich Countries — Universal Gains',
             fontsize=15, fontweight='bold', y=1.01)
plt.tight_layout()
...
if len(us) > 5:
    ...
    ax.set_title(f'United States: Growth Indexed to {base_year}=100', fontsize=12, fontweight='bold')
    ax.set_ylabel(f'Real income index ({base_year}=100)')
    ax.legend(fontsize=10)
    ax.annotate('Top 20% grew FASTER,\nbut bottom 20% still grew',
                xy=(2015, 110), fontsize=10,
                bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
...
if len(fr) > 5:
    ...
    ax.axhline(y=100, color='grey', linewidth=0.5, linestyle='--')
    ax.set_title(f'France: Growth Indexed to {base_year}=100', fontsize=12, fontweight='bold')
    ax.set_ylabel(f'Real income index ({base_year}=100)')
    ax.legend(fontsize=10)
    ax.annotate('More equal growth,\nbut ALL quintiles still grew',
                xy=(2010, 110), fontsize=10,
                bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))

plt.suptitle('Chart 51: The Real Story — Everyone Gained, But Unequally',
             fontsize=15, fontweight='bold', y=1.01)
plt.tight_layout()

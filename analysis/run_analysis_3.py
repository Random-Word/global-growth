...
for pct_label, color in colors_pct.items():
    ...
    ax.plot(
        data["year"], data["gdppc"], "-", color=color, linewidth=2.5, label=pct_label
    )
...
for yr in [1950, 1970, 1990, 2000, 2010, 2022]:
    ...
    if len(yr_data) >= 40:
        ...
        ratio = p90 / p5
        print(
            f"    {yr}: P5=${p5:,.0f}  P10=${p10:,.0f}  P20=${p20:,.0f}  P50=${p50:,.0f}  P90=${p90:,.0f}  (90/5 ratio: {ratio:.1f}x)"
        )
...
for yr in [1960, 1980, 2000, 2010, 2022]:
    ...
    if len(yr_records) >= 4:
        vals = {
            r["percentile"]: (r["gdppc"], r["country"])
            for _, r in yr_records.iterrows()
        }
        parts = []
...
for pct_label in ["P5 (floor)", "P10", "P20", "P50 (median)", "P90"]:
    ...
    if len(data) > 10:
        for period_start, period_end, period_label in [
            (1950, 1980, "1950-80"),
            (1980, 2000, "1980-00"),
            (2000, 2022, "2000-22"),
        ]:
            ...
            if len(start_val) > 0 and len(end_val) > 0 and start_val[0] > 0:
                annual_g = (
                    (end_val[0] / start_val[0]) ** (1 / (period_end - period_start)) - 1
                ) * 100
...
for col, color, ls in [
    ...
    ax.plot(
        df_ratios["year"], df_ratios[col], ls, color=color, linewidth=2.5, label=col
    )
...
fig.suptitle(
    "The 'Good Life' Threshold: Where Do Returns to Income Diminish?\nEach dot = one country-year",
    fontsize=16,
    fontweight="bold",
)
...
ax = axes[0][0]
data = wdi_recent[
    wdi_recent["life_expectancy"].notna() & wdi_recent["gdppc_ppp_current"].notna()
].copy()
...
data["gdp_bin"] = pd.cut(
    data["gdppc_ppp_current"],
    bins=[0, 1000, 2000, 3000, 5000, 7500, 10000, 15000, 20000, 30000, 50000, 100000],
)
binned = data.groupby("gdp_bin", observed=True)["life_expectancy"].agg(
    ["mean", "count"]
)
...
ax = axes[0][1]
data2 = wdi_recent[
    wdi_recent["under5_mortality"].notna() & wdi_recent["gdppc_ppp_current"].notna()
].copy()
data2 = data2[data2["gdppc_ppp_current"] > 0]
ax.scatter(
    data2["gdppc_ppp_current"],
    data2["under5_mortality"],
    alpha=0.15,
    s=10,
    color="steelblue",
)

data2["gdp_bin"] = pd.cut(
    data2["gdppc_ppp_current"],
    bins=[0, 1000, 2000, 3000, 5000, 7500, 10000, 15000, 20000, 30000, 50000, 100000],
)
...
ax.scatter(
    pip_gini["annual_mean"], pip_gini["gini"] * 100, alpha=0.3, s=20, color="steelblue"
)
ax.set_xlabel("Mean Daily Income ($/day from PIP)")
ax.set_ylabel("Gini Coefficient (%)")
ax.set_title(
    "Inequality vs Income Level\n(Does inequality naturally fall with growth?)"
)
# Regression
...
if len(valid_gini) > 20:
    ...
    print(
        f"  {'Higher income → less inequality' if slope < 0 else 'No clear relationship (or more inequality)'}"
    )
...
for yr_range, color, label in [
    ...
    if len(period_data) > 0:
        bins = pd.cut(
            period_data["gdppc_ppp_current"],
            bins=[0, 2000, 5000, 10000, 20000, 50000, 150000],
        )
...
for yr_range in [1990, 2000, 2010, 2020]:
    ...
    pd_data = wdi[
        (wdi["year"] >= yr_range)
        & (wdi["year"] <= yr_end)
        & wdi["gdppc_ppp_current"].between(2000, 5000)
        & wdi["life_expectancy"].notna()
    ]
    if len(pd_data) > 0:
        print(
            f"    {yr_range}s: {pd_data['life_expectancy'].mean():.1f} years (n={len(pd_data)})"
        )
...
if len(df_floor) > 0:
    ax.plot(
        df_floor["year"],
        df_floor["bottom_20_le"],
        "r-",
        linewidth=2.5,
        label="Bottom 20% of countries",
    )
    ax.plot(
        df_floor["year"],
        df_floor["median_le"],
...
ax.set_title(
    "Life Expectancy: Floor, Median, Ceiling\n(All rising — convergence in WELFARE)"
)
ax.set_ylabel("Life Expectancy (years)")
ax.set_xlabel("Year")
ax.legend(fontsize=10)
...
for yr in [1960, 1980, 2000, 2022]:
    ...
    if len(row) > 0:
        ...
        print(
            f"    {yr}: bottom 20% = {r['bottom_20_le']:.1f}, median = {r['median_le']:.1f}, top 20% = {r['top_20_le']:.1f}, gap = {gap:.1f} years"
        )
...
wdi_test = wdi_test[
    wdi_test["gdppc_ppp_current"].notna() & (wdi_test["gdppc_ppp_current"] > 0)
]
...
for t in thresholds:
    ...
    if len(above) > 0:
        le_good = (
            above[above["life_expectancy"] >= 70].shape[0]
            / above[above["life_expectancy"].notna()].shape[0]
            * 100
            if above["life_expectancy"].notna().sum() > 0
            else 0
        )
        cm_good = (
            above[above["under5_mortality"] <= 25].shape[0]
            / above[above["under5_mortality"].notna()].shape[0]
            * 100
            if above["under5_mortality"].notna().sum() > 0
            else 0
        )
...
for _, r in df_gl.iterrows():
    print(
        f"  ${r['threshold']/1000:>5.0f}k      {r['pct_le_70plus']:>5.1f}%      {r['pct_cm_under25']:>5.1f}%"
    )
...
for cc in gini_data["country_code"].unique():
    ...
    gdp_start = wdi[
        (wdi["country_code"] == cc)
        & (wdi["year"] == first["year"])
        & (wdi["gdppc_constant_2015usd"].notna())
    ]
    gdp_end = wdi[
        (wdi["country_code"] == cc)
        & (wdi["year"] == last["year"])
    ...
    if (
        len(gdp_start) > 0
        and len(gdp_end) > 0
        and gdp_start.iloc[0]["gdppc_constant_2015usd"] > 0
    ):
        g_growth = (
            (
                gdp_end.iloc[0]["gdppc_constant_2015usd"]
                / gdp_start.iloc[0]["gdppc_constant_2015usd"]
            )
            ** (1 / span)
            - 1
        ) * 100
...
if len(df_gg) > 10:
    ...
    print(
        f"  Interpretation: {'Faster growth → more inequality' if slope > 0 else 'Faster growth → less inequality'} ({'significant' if p < 0.05 else 'not significant'})"
    )
...
for yr in range(1960, 2025, 5):
    ...
    cm = wdi_yr[wdi_yr["under5_mortality"].notna()].sort_values(
        "under5_mortality", ascending=False
    )
    if len(cm) >= 20:
        n20 = max(len(cm) // 5, 1)
...
l3 = ax2.plot(
    ...
    linewidth=2.5,
    markersize=6,
    label="Life Exp bottom 20% (right)",
)
...
ax1.set_title(
    "Floor-Raising Scorecard: Absolute Conditions for the World's Poorest Countries",
    fontsize=14,
    fontweight="bold",
)

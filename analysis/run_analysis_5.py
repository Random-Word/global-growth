...
groups_data = {
    "High income": hi_agg,
    "Upper-middle": umi_agg,
    "Lower-middle": lmi_agg,
    "World": world,
}
bar_data = []
for label, df in groups_data.items():
    ...
    for start, end in decades:
        if start in d.index and end in d.index:
            ...
            bar_data.append(
                {"Group": label, "Decade": f"{start}s", "Annual change %": rate}
            )
...
plt.suptitle(
    "Chart 19: Carbon & Energy Decoupling Trends",
    fontsize=15,
    fontweight="bold",
    y=1.01,
)
...
for label, df in [
    ...
    if 1990 in vals and 2020 in vals:
        ...
        print(
            f"  {label:20s}: {vals.get(1990, 'n/a'):.3f}(1990) → {vals.get(2020, 'n/a'):.3f}(2020)  "
...
for label, df in [("World", world), ("High income", hi_agg)]:
    d = df[(df["co2_per_gdp"].notna()) & (df["year"] >= 1970)].set_index("year")[
        "co2_per_gdp"
    ]
    for p1, p2 in [((1970, 1990), (1990, 2010)), ((1990, 2000), (2010, 2020))]:
        ...
        if all(y in d.index for y in [s1, e1, s2, e2]):
            ...
            print(
                f"  {label}: {s1}-{e1}: {r1:.2f}%/yr → {s2}-{e2}: {r2:.2f}%/yr  "
...
w = (
    world[world["year"] == 2022].iloc[0]
    if len(world[world["year"] == 2022]) > 0
    else world[world["year"] == 2021].iloc[0]
)
w_yr = int(w["year"])
...
for label, df in [
    ...
    if len(row) > 0:
        r = row.iloc[0]
        print(
            f"  {label:20s}: CO₂={r['co2']:.0f} Mt  ({r['co2']/total_co2_world*100:.1f}%)  "
...
non_hi_pop = sum(
    latest[g]["population"]
    for g in ["Upper-middle", "Lower-middle", "Low income"]
...
print(
    f"  Global CO₂: {scenario_a_co2:.0f} Mt ({scenario_a_co2/1000:.1f} Gt)  vs current {total_co2_world/1000:.1f} Gt"
)
...
hi_gdp_pc = (
    latest["High income"]["gdp"] / latest["High income"]["population"]
    if latest["High income"]["population"] > 0
    else 50000
)
...
for decouple_rate, label in [
    (0.02, "2%/yr"),
    (0.025, "2.5%/yr"),
    (0.035, "3.5%/yr (accelerated)"),
]:
    for years in [50, 100, 200]:
        ...
        future_global_co2 = (
            (latest["High income"]["population"] + non_hi_pop) * future_hi_co2_pc / 1e6
        )
        print(
            f"  Decoupling {label}, after {years:3d}yr: CO₂/cap={future_hi_co2_pc:.2f}t  "
...
print(
    f"  1.5°C budget remaining: ~{remaining_15c} Gt → {remaining_15c/current_annual:.0f} years at current rates"
)
print(
    f"  2°C budget remaining:   ~{remaining_2c} Gt → {remaining_2c/current_annual:.0f} years at current rates"
)
...
print(
    f"  Rest of world: {non_hi_current_co2:.1f} Gt ({non_hi_current_co2/current_annual*100:.0f}%)"
)
...
for decouple_rate in [0.02, 0.03, 0.05]:
    ...
    for yr in range(1, 201):
        annual_non_hi = (
            non_hi_current_co2 * ((1 + gdp_growth) ** yr) * ((1 - decouple_rate) ** yr)
        )
...
for dk, grp in sdf.groupby("decouple"):
    ...
    for _, row in grp.iterrows():
        budget_status_2c = (
            "WITHIN 2°C"
            if row["cumul_gt"] < remaining_2c
            else f"EXCEEDS 2°C by {row['cumul_gt']-remaining_2c:.0f} Gt"
        )
        print(
            f"    Year {row['year']:3.0f}: Annual={row['annual_gt']:.1f} Gt  Cumulative={row['cumul_gt']:.0f} Gt  "
...
if len(tdf) > 0:
    ...
    ax.legend()
    ax.set_title(
        f"Production vs Consumption CO₂ ({w_yr})", fontsize=12, fontweight="bold"
    )
...
ax.axhline(y=0, color="black", linewidth=0.5)
ax.set_title(
    "Net CO₂ Trade (+ = net importer/offshorer)", fontsize=12, fontweight="bold"
)
...
for label, df, color in [
    ...
    if len(d) > 0:
        ...
        d = df[
            (df["year"] >= 1990) & (df["co2"].notna()) & (df["consumption_co2"].notna())
        ].sort_values("year")
        if len(d) > 0:
            ax.plot(
                d["year"],
                d["consumption_co2"] - d["co2"],
                label=label,
                color=color,
                linewidth=2,
            )
...
ax.legend(fontsize=9)
ax.annotate(
    "Positive = consuming more\nthan producing (offshoring)",
    xy=(0.02, 0.95),
...
ax = axes[1, 1]
for label, metric, color, ls in [
    ("US production", "co2_per_capita", "blue", "-"),
    ...
    d = co2[
        (co2["country"] == country_name) & (co2["year"] >= 1990) & (co2[metric].notna())
    ]
    d = d.sort_values("year")
    if len(d) > 0:
        ax.plot(
            d["year"], d[metric], label=label, color=color, linestyle=ls, linewidth=1.5
        )
ax.set_title(
    "Per Capita CO₂: Production vs Consumption", fontsize=12, fontweight="bold"
)
ax.set_ylabel("t CO₂ per capita")
ax.legend(fontsize=9)

plt.suptitle(
    'Chart 20: CO₂ Trade Flows & "Exploitation" Test',
    fontsize=15,
    fontweight="bold",
    y=1.01,
)
...
for c in ["United States", "United Kingdom", "Germany", "Japan", "China", "India"]:
    ...
    if len(d) > 0:
        ...
        if pd.notna(prod) and pd.notna(cons):
            ...
            print(
                f"  {c:20s}: Production={prod:8.0f} Mt  Consumption={cons:8.0f} Mt  "
...
for yr in [1990, 2000, 2010, 2020, w_yr]:
    ...
    if len(d) > 0:
        ...
        if pd.notna(prod_pc) and pd.notna(cons_pc):
            ...
            print(
                f"  {yr}: Production={prod_pc:.1f}t  Consumption={cons_pc:.1f}t  Gap={gap:+.1f}%"
            )
...
ax.set_title(
    "Absolute Decoupling: GDP vs CO₂ (1990=100)", fontsize=12, fontweight="bold"
)
...
for c in test_countries:
    ...
    if len(d) > 5:
        ...
        still_rising = (
            peak_year >= latest["year"] - 3
        )  # peaked within last 3 years = still rising
        peak_data.append(
            {
                "country": c,
...
ax.set_title(
    "When Did CO₂ Peak? (🟢peaked  🔴still rising)", fontsize=12, fontweight="bold"
)
...
for label, df, color in [
    ...
    d = (
        df[(df["co2_per_gdp"].notna()) & (df["year"] >= 1960)]
        .sort_values("year")
        .set_index("year")
    )
    if len(d) > 15:
        ...
        rolling_rate = intensity.pct_change(10).apply(
            lambda x: ((1 + x) ** (1 / 10) - 1) * 100 if pd.notna(x) else np.nan
        )
        ax.plot(rolling_rate.index, rolling_rate.values, label=label, color=color)
ax.axhline(y=0, color="black", linewidth=0.5)
ax.axhline(
    y=-3.5, color="red", linewidth=1, linestyle=":", label="Needed for 1.5°C (~3.5%/yr)"
)
ax.set_title("Rolling 10yr CO₂/GDP Decoupling Rate", fontsize=12, fontweight="bold")
ax.set_ylabel("Annual change in CO₂ intensity (%)")
ax.legend(fontsize=9)
ax.annotate(
    "Below zero = decoupling\nMore negative = faster",
    xy=(0.02, 0.05),
    xycoords="axes fraction",
    fontsize=9,
...
for budget_label, budget_gt, color in [
    ("1.5°C (300 Gt)", 300, "red"),
    ...
    for g in growth_rates:
        ...
        for _ in range(50):
            ...
            cumul = sum(
                current_annual * ((1 + g) * (1 - d_mid)) ** t for t in range(50)
            )
            if cumul > budget_gt:
                d_low = d_mid
            else:
    ...
    ax.plot(
        growth_rates * 100, required_d, label=budget_label, color=color, linewidth=2
    )

ax.axhline(y=2.5, color="gray", linewidth=1, linestyle=":", alpha=0.7)
ax.annotate(
    "Current best decoupling rate (~2.5%/yr)", xy=(3, 2.7), fontsize=8, color="gray"
)
ax.set_xlabel("Global GDP Growth Rate (%/yr)")
ax.set_ylabel("Required CO₂ Intensity Decline (%/yr)")
ax.set_title(
    "Required Decoupling to Stay Within Carbon Budgets\n(over next 50 years)",
    fontsize=12,
    fontweight="bold",
)
ax.legend(fontsize=9)
...
plt.suptitle(
    "Chart 21: Absolute Decoupling & Climate Constraints",
    fontsize=15,
    fontweight="bold",
    y=1.01,
)
...
for _, r in pdf.iterrows():
    status = (
        "STILL RISING"
        if r["still_rising"]
        else f"down {r['decline_from_peak']:.0f}% from peak"
    )
...
print(
    f"  Below threshold: {below_gl_pop/1e9:.2f} billion people ({below_gl_pop/(below_gl_pop+above_gl_pop)*100:.0f}%)"
)
print(f"      Avg GDP/cap: ${below_gl_gdp/below_gl_pop:,.0f}")
print(f"      Total GDP: ${below_gl_gdp/1e12:.1f} trillion")
print(
    f"  Above threshold: {above_gl_pop/1e9:.2f} billion people ({above_gl_pop/(below_gl_pop+above_gl_pop)*100:.0f}%)"
)
...
poor_co2_data = co2[
    (co2["iso_code"].isin(poor_countries))
    & (co2["year"] == w_yr)
    & (co2["co2"].notna())
]
...
print(
    f"  Current poor-world CO₂: ~{poor_co2_actual:.0f} Mt ({poor_co2_actual/total_co2_world*100:.0f}% of global)"
)
print(
    f"  Current rich-world CO₂: ~{rich_co2_actual:.0f} Mt ({rich_co2_actual/total_co2_world*100:.0f}% of global)"
)
...
for decouple, ls in [(0.02, ":"), (0.03, "--"), (0.04, "-"), (0.05, "-.")]:
    ...
    ax.plot(
        2022 + years_forward,
        poor_emissions,
        label=f"Decouple {decouple*100:.0f}%/yr",
        linestyle=ls,
    )

ax.axhline(y=current_annual, color="black", linewidth=0.5, linestyle=":")
ax.annotate(
    f"Current: {current_annual:.0f} Gt", xy=(2025, current_annual + 1), fontsize=9
)
ax.set_xlabel("Year")
ax.set_ylabel("Global CO₂ (Gt/yr)")
ax.set_title(
    "Annual CO₂: Rich World Frozen, Poor World Grows 5%/yr",
    fontsize=12,
    fontweight="bold",
)
ax.legend()
...
plt.suptitle(
    "Chart 22: Poor-World-Only Growth Scenario", fontsize=15, fontweight="bold", y=1.02
)
plt.tight_layout()
plt.savefig(
    f"{CHART_DIR}/22_poor_world_growth_scenario.png", dpi=150, bbox_inches="tight"
)
plt.close()
...
for decouple in [0.02, 0.03, 0.04, 0.05]:
    ...
    print(
        f"  Decouple {decouple*100:.0f}%/yr: 1.5°C exceeded in {exceeded_15c if exceeded_15c else '>100'}yr  "
...
for label, df, color in [
    ...
    d = df[(df["year"] >= 1965) & (df["co2_per_unit_energy"].notna())].sort_values(
        "year"
    )
    if len(d) > 0:
        ax.plot(d["year"], d["co2_per_unit_energy"], label=label, color=color)
ax.set_title(
    "CO₂ per Unit Energy (kg/kWh)\nHow Clean is the Energy Mix?",
    fontsize=12,
    fontweight="bold",
)
...
ax.axhline(y=100, color="black", linewidth=0.5, linestyle=":")
ax.set_title(
    "Energy Intensity of GDP (normalized, earliest=100)\nHow Much Energy per $?",
    fontsize=12,
    fontweight="bold",
)
ax.set_ylabel("Index")
...
plt.suptitle(
    "Chart 23: Energy Transition Progress", fontsize=15, fontweight="bold", y=1.02
)
plt.tight_layout()
...
for label, df in [("World", world), ("High income", hi_agg)]:
    d = df[
        df["co2_per_gdp"].notna()
        & df["co2_per_unit_energy"].notna()
        & df["energy_per_gdp"].notna()
    ]
    d = d.set_index("year")
    for p in [(2000, 2010), (2010, 2020)]:
        if p[0] in d.index and p[1] in d.index:
            ...
            print(
                f"    CO₂/GDP: {co2_gdp_rate:.2f}%/yr = CO₂/Energy: {co2_energy_rate:.2f}%/yr + Energy/GDP: {energy_gdp_rate:.2f}%/yr"
            )
            pct_from_clean = (
                co2_energy_rate / co2_gdp_rate * 100 if co2_gdp_rate != 0 else 0
            )
            pct_from_eff = (
                energy_gdp_rate / co2_gdp_rate * 100 if co2_gdp_rate != 0 else 0
            )
            print(
                f"    → {pct_from_clean:.0f}% from cleaner energy, {pct_from_eff:.0f}% from efficiency"
            )

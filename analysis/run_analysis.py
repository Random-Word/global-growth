...
for pl in [2.15, 3.65, 6.85, 10.0]:
    ...
    print(f"  PIP {pl}: {len(df)} country-years, {len(pip_regional[key])} regional-years")
...
fig.suptitle("Has Growth Made Redistribution Cheaper?\nPoverty Gap as % of Global GDP Over Time", fontsize=16, fontweight='bold')
...
for idx, pl in enumerate([2.15, 3.65, 6.85, 10.0]):
    ...
    # Sum across regions for each year
    yearly = reg.groupby('reporting_year').agg({
        'pop_in_poverty': 'sum',
        'reporting_pop': 'sum',
        'poverty_gap': lambda x: np.nan,  # placeholder
        'headcount': lambda x: np.nan,
    }).reset_index()
    
    # Also compute actual total poverty gap in dollars
    # poverty_gap index = (1/N) * sum of (z-yi)/z for poor
    # Total gap in dollars/year = sum over regions of: poverty_gap_index * poverty_line * pop * 365
    gap_records = []
    for yr in reg['reporting_year'].unique():
        yr_data = reg[reg['reporting_year'] == yr]
        total_gap_usd = 0
        total_pop = 0
        total_poor = 0
        for _, row in yr_data.iterrows():
            # poverty_gap is the Foster-Greer-Thorbecke P1 index
            # Total shortfall = P1 * poverty_line * total_population * 365
            region_gap = row['poverty_gap'] * pl * row['reporting_pop'] * 365
            total_gap_usd += region_gap
            total_pop += row['reporting_pop']
            total_poor += row.get('pop_in_poverty', 0)
        
        global_gdp = gdp_by_year.get(yr, np.nan)
        gap_pct = (total_gap_usd / global_gdp * 100) if global_gdp and not np.isnan(global_gdp) else np.nan
    ...
    l1 = ax.plot(df_gap['year'], df_gap['gap_pct_gdp'], 'b-o', markersize=3, label='Gap as % of GDP')
    l2 = ax2.plot(df_gap['year'], df_gap['total_gap_bn'], 'r--s', markersize=3, alpha=0.7, label='Gap (billion $)')
    ...
    if len(df_gap) >= 2:
        ...
        last = df_gap.iloc[-1]
        print(f"\n  ${pl}/day line:")
        print(f"    {int(first['year'])}: gap = ${first['total_gap_bn']:.0f}B = {first['gap_pct_gdp']:.2f}% of GDP, {first['total_poor_bn']:.2f}B people")
        print(f"    {int(last['year'])}: gap = ${last['total_gap_bn']:.0f}B = {last['gap_pct_gdp']:.2f}% of GDP, {last['total_poor_bn']:.2f}B people")
        print(f"    Change: {last['gap_pct_gdp'] - first['gap_pct_gdp']:+.2f} ppt ({(last['gap_pct_gdp']/first['gap_pct_gdp'] - 1)*100:+.1f}%)")
...
for (key, df_gap), color in zip(results_a1.items(), colors):
    ax.plot(df_gap['year'], df_gap['gap_pct_gdp'], '-o', color=color, markersize=3, label=f"${key}/day")
ax.set_title("Poverty Gap as % of Global GDP: It Has Never Been Cheaper to End Poverty", fontsize=14, fontweight='bold')
ax.set_xlabel("Year")
...
fig.suptitle("Where Has Poverty Reduction Come From?\nPeople Lifted Above Each Poverty Line by Region", fontsize=16, fontweight='bold')
...
for idx, pl in enumerate([2.15, 3.65, 6.85, 10.0]):
    ...
    for _, grp in reg.groupby('region_code'):
        ...
        ax.plot(data['reporting_year'], data['pop_in_poverty'] / 1e9, '-', linewidth=2, label=rname[:30])
...
key_countries = {
    ...
    'BGD': 'Bangladesh', 'VNM': 'Vietnam', 'NGA': 'Nigeria',
    'ETH': 'Ethiopia', 'COD': 'DR Congo', 'BRA': 'Brazil',
}
...
for cc, name in key_countries.items():
    ...
    if len(cdata) > 0:
        ...
        ax.plot(cdata['reporting_year'], poor / 1e6, '-o', markersize=3, linewidth=2, label=name)
        
        if len(cdata) >= 2:
            first_yr = cdata.iloc[0]
            last_yr = cdata.iloc[-1]
            change = (last_yr['headcount'] * last_yr['reporting_pop'] - first_yr['headcount'] * first_yr['reporting_pop']) / 1e6
            print(f"  {name}: {change:+.0f}M people at $2.15/day ({int(first_yr['reporting_year'])}-{int(last_yr['reporting_year'])})")

ax.set_title("People in Extreme Poverty ($2.15/day) by Key Country", fontsize=14, fontweight='bold')
ax.set_xlabel("Year")
...
world_data = reg215.groupby('reporting_year').agg({'pop_in_poverty': 'sum'}).reset_index()
china_data = pip215[pip215['country_code'] == 'CHN'].sort_values('reporting_year')

# Find overlapping years
common_start = max(world_data['reporting_year'].min(), china_data['reporting_year'].min())
...
for start_yr, end_yr in [(1990, 2010), (1990, 2019), (2000, 2019)]:
    w_start = world_data[world_data['reporting_year'] == start_yr]['pop_in_poverty'].values
...
for cc, (name, color) in growth_cases.items():
    ...
    ax.plot(cdata['years_since_t0'], cdata['gdppc_ratio'], color=color, linewidth=2, label=f"{name} (t₀={int(t0_year)})")
    print(f"  {name}: crossed ${BASE_THRESHOLD}/yr in {int(t0_year)}")

ax.set_yscale('log')
ax.set_title(f"Growth After Crossing ${BASE_THRESHOLD}/yr (≈$2.75/day)\nTime Normalized", fontweight='bold')
...
print(f"  {'Country':<15} {'t₀ year':<10} {'→ 5x':<10} {'→ 10x':<10} {'→ 20x':<10} {'→ 40x':<10}")
for cc, (name, color) in growth_cases.items():
    cdata = mad[(mad['countrycode'] == cc) & (mad['gdppc'].notna())].sort_values('year')
    above = cdata[cdata['gdppc'] >= BASE_THRESHOLD]
    if len(above) == 0:
...
fig, axes = plt.subplots(2, 2, figsize=(16, 12))
fig.suptitle("Convergence Analysis: Are Poor Countries Catching Up?", fontsize=16, fontweight='bold')
...
for yr in range(1950, 2023):
    ...
    if len(yr_data) >= 30:  # need enough countries
        ...
        sigma_records.append({
            ...
            'sigma_weighted': np.sqrt(np.average((log_gdppc - np.average(log_gdppc, weights=yr_data['pop']))**2, weights=yr_data['pop']))
        })

df_sigma = pd.DataFrame(sigma_records)
ax.plot(df_sigma['year'], df_sigma['sigma_unweighted'], 'b-', linewidth=2, label='Unweighted')
ax.plot(df_sigma['year'], df_sigma['sigma_weighted'], 'r-', linewidth=2, label='Population-weighted')
ax.set_title("Sigma Convergence: Dispersion of log(GDP/capita)")
ax.set_xlabel("Year")
ax.set_ylabel("Std. dev. of log(GDP per capita)")
ax.legend()
print(f"  Sigma convergence (unweighted): {df_sigma.iloc[0]['sigma_unweighted']:.3f} ({int(df_sigma.iloc[0]['year'])}) -> {df_sigma.iloc[-1]['sigma_unweighted']:.3f} ({int(df_sigma.iloc[-1]['year'])})")
print(f"  Sigma convergence (pop-weighted): {df_sigma.iloc[0]['sigma_weighted']:.3f} ({int(df_sigma.iloc[0]['year'])}) -> {df_sigma.iloc[-1]['sigma_weighted']:.3f} ({int(df_sigma.iloc[-1]['year'])})")

# Panel B: Beta convergence scatter (initial GDP vs growth rate)
ax = axes[0][1]
# Use 1990-2022 period
start_yr, end_yr = 1990, 2022
start_data = mad[(mad['year'] == start_yr) & (mad['gdppc'].notna()) & (mad['gdppc'] > 0)].set_index('countrycode')
end_data = mad[(mad['year'] == end_yr) & (mad['gdppc'].notna()) & (mad['gdppc'] > 0)].set_index('countrycode')
common = start_data.index.intersection(end_data.index)

if len(common) > 10:
    log_initial = np.log(start_data.loc[common, 'gdppc'])
    growth = (np.log(end_data.loc[common, 'gdppc']) - np.log(start_data.loc[common, 'gdppc'])) / (end_yr - start_yr) * 100
    ...
    print(f"    {'Convergence' if slope < 0 else 'Divergence'} ({'significant' if p < 0.05 else 'not significant'})")
...
for (sy, ey), clr in zip(periods, colors_p):
    sd = mad[(mad['year'] == sy) & (mad['gdppc'].notna()) & (mad['gdppc'] > 0)].set_index('countrycode')
    ed = mad[(mad['year'] == ey) & (mad['gdppc'].notna()) & (mad['gdppc'] > 0)].set_index('countrycode')
...
for yr in range(1960, 2020, 5):
    ...
    merged = yr_start[yr_start['countrycode'].isin(common_bt)].merge(
        yr_end[yr_end['countrycode'].isin(common_bt)], on='countrycode', suffixes=('_s', '_e'))
...
ax.plot(df_bt['year'], df_bt['bottom_20pct_growth'], 'r-o', label='Poorest 20% of countries', markersize=4)
ax.plot(df_bt['year'], df_bt['top_20pct_growth'], 'b-s', label='Richest 20% of countries', markersize=4)
...
pip_merged = pip215.merge(wdi_gdp, left_on=['country_code', 'reporting_year'], 
                          right_on=['country_code', 'reporting_year'], how='inner')
...
for _, row_start in pip_merged.iterrows():
    ...
    future = pip_merged[(pip_merged['country_code'] == cc) & 
    ...
    if h_s > 0.01 and h_e > 0.001 and g_s > 0 and g_e > 0:
        ...
        if abs(pct_change_gdp) > 0.01:
            ...
            elasticity_records.append({
                'country_code': cc,
                'region': row_start.get('region_code', row_start.get('region_name', '')),
                'start_year': yr_s,
...
if region_elasticities:
    bars = ax.barh(list(region_elasticities.keys()), list(region_elasticities.values()), color='steelblue')
    ax.set_xlabel("Median Growth Elasticity of Poverty")
    ax.set_title("Elasticity by Region\n(more negative = growth reduces poverty faster)")
...
fig, axes = plt.subplots(2, 2, figsize=(16, 12))
fig.suptitle("The Cost of Ending Poverty: Redistribution vs. Growth", fontsize=16, fontweight='bold')

# Panel A: Poverty gap at $6.85/day with context lines
ax = axes[0][0]
gap685 = results_a1.get('6.85')
if gap685 is not None and len(gap685) > 0:
    ax.fill_between(gap685['year'], 0, gap685['total_gap_bn'], alpha=0.3, color='red', label='Poverty gap ($6.85/day)')
    ax.plot(gap685['year'], gap685['total_gap_bn'], 'r-', linewidth=2)
    
    # Add context: global military spending (~$2,000B in 2024)
    ax.axhline(y=2000, color='gray', linestyle='--', alpha=0.7, label='Global military spending (~$2T)')
...
ax = axes[0][1]
for pl, color, label in [(2.15, 'blue', '$2.15/day'), (3.65, 'orange', '$3.65/day'), 
                          (6.85, 'red', '$6.85/day'), (10.0, 'purple', '$10/day')]:
    key = str(pl)
    gap = results_a1.get(key)
    if gap is not None:
        # Perfect targeting cost
        ax.plot(gap['year'], gap['total_gap_bn'], '-', color=color, linewidth=1, alpha=0.5)
        # Realistic targeting (3x)
        ax.plot(gap['year'], gap['total_gap_bn'] * 3, '--', color=color, linewidth=2, label=f'{label} (3x targeting)')
...
for pl, color in [(2.15, 'blue'), (3.65, 'orange'), (6.85, 'red')]:
    ...
    if gap is not None:
        ...
        gap_copy = gap.copy()
        gap_copy['years_needed'] = gap_copy['total_gap_bn'] / (0.0015 * gap_copy['global_gdp_tn'] * 1000)
        ax.plot(gap_copy['year'], gap_copy['years_needed'], '-o', color=color, markersize=3,
                label=f'${pl}/day')

ax.set_title("Years of 3% Growth Needed to Close Gap\n(if poorest 60% get 5% of new income)")
ax.set_xlabel("Year")
...
print(f"  {'Line':<12} {'People':<12} {'Perfect Gap':<15} {'Realistic(3x)':<15} {'% GDP':<10} {'Realistic % GDP':<15}")
for row in table_data:
    print(f"  {row[0]:<12} {row[1]:<12} {row[2]:<15} {row[3]:<15} {row[4]:<10} {row[5]:<15}")
...
rotation_countries = {
    ...
    'CHN': 'China', 'IND': 'India', 'VNM': 'Vietnam',
    'THA': 'Thailand', 'MYS': 'Malaysia', 'IDN': 'Indonesia',
    'BGD': 'Bangladesh', 'ETH': 'Ethiopia', 'RWA': 'Rwanda',
}
...
for cc, name in rotation_countries.items():
    ...
    for ds, de in decades:
        start = mad[(mad['countrycode'] == cc) & (mad['year'] == ds) & (mad['gdppc'].notna())]
        end = mad[(mad['countrycode'] == cc) & (mad['year'] == de) & (mad['gdppc'].notna())]
        if len(start) > 0 and len(end) > 0 and start.iloc[0]['gdppc'] > 0:
            annual_growth = ((end.iloc[0]['gdppc'] / start.iloc[0]['gdppc']) ** (1/(de-ds)) - 1) * 100
...
for name, growths in decade_growth.items():
    ...
    if valid:
        ax.plot([d for d, g in valid], [g for d, g in valid], '-o', linewidth=2, label=name, markersize=4)
...
mfg_countries = {
    ...
    'IND': 'India', 'VNM': 'Vietnam', 'THA': 'Thailand',
...
if 'manufacturing_va_pct' in wdi_sub.columns:
    for cc, name in mfg_countries.items():
        cdata = wdi_sub[(wdi_sub['country_code'] == cc) & (wdi_sub['manufacturing_va_pct'].notna())].sort_values('year')
        if len(cdata) > 0:
            ax.plot(cdata['year'], cdata['manufacturing_va_pct'], '-', linewidth=2, label=name)
else:
    ax.text(0.5, 0.5, "Manufacturing data\nnot available\n(download timed out)", 
...
for ds, de in decades:
    ...
    for cc in mad['countrycode'].unique():
        ...
        if len(start) > 0 and len(end) > 0 and start.iloc[0]['gdppc'] > 100:
            annual_g = ((end.iloc[0]['gdppc'] / start.iloc[0]['gdppc']) ** (1/(de-ds)) - 1) * 100
...
for _, row in poorest_20.iterrows():
    cc = row['countrycode']
    cdata = mad[(mad['countrycode'] == cc) & (mad['gdppc'].notna()) & (mad['year'] >= 1990)].sort_values('year')
    if len(cdata) > 2:
        ax.plot(cdata['year'], cdata['gdppc'], '-', linewidth=1.5, alpha=0.7, label=row['country'][:15])
...
escaped = poor_1960.merge(rich_2022[['countrycode', 'gdppc']], on='countrycode', suffixes=('_1960', '_2022'))
escaped['growth_factor'] = escaped['gdppc_2022'] / escaped['gdppc_1960']
escaped['annual_growth'] = ((escaped['gdppc_2022'] / escaped['gdppc_1960']) ** (1/62) - 1) * 100

fig, ax = plt.subplots(figsize=(14, 8))
escaped_sorted = escaped.sort_values('growth_factor', ascending=True)
colors_esc = ['green' if g > 5 else 'orange' if g > 2 else 'red' for g in escaped_sorted['growth_factor']]
ax.barh(escaped_sorted['country'][:40], escaped_sorted['growth_factor'][:40], color=colors_esc[:40])
ax.set_xlabel("GDP per capita multiple (2022 vs 1960)")
ax.set_title("Countries Poor in 1960 (< $3000 GDP/capita): Who Escaped?", fontweight='bold')
...
for _, r in top.iterrows():
    print(f"    {r['country']}: ${r['gdppc_1960']:.0f} -> ${r['gdppc_2022']:.0f} ({r['growth_factor']:.1f}x, {r['annual_growth']:.1f}%/yr)")
...
fig.suptitle("Global Development: Growth, Poverty, and Redistribution\nSummary Dashboard", fontsize=18, fontweight='bold')

# 1. Poverty headcount over time at multiple thresholds
ax = axes[0][0]
for pl, color, label in [(2.15, '#2196F3', '$2.15/day'), (3.65, '#FF9800', '$3.65/day'), 
                          (6.85, '#E91E63', '$6.85/day')]:
    key = str(pl)
    reg = pip_regional[key]
    yearly = reg.groupby('reporting_year').agg({'pop_in_poverty': 'sum', 'reporting_pop': 'sum'}).reset_index()
    yearly['rate'] = yearly['pop_in_poverty'] / yearly['reporting_pop'] * 100
    ax.plot(yearly['reporting_year'], yearly['rate'], '-', color=color, linewidth=2, label=label)
...
for key, color in [('2.15', '#2196F3'), ('3.65', '#FF9800'), ('6.85', '#E91E63')]:
    gap = results_a1.get(key)
    if gap is not None:
        ax.plot(gap['year'], gap['gap_pct_gdp'], '-', color=color, linewidth=2, label=f'${key}/day')
...
ax = axes[0][2]
ax.plot(df_sigma['year'], df_sigma['sigma_unweighted'], 'b-', linewidth=2, label='Unweighted')
ax.plot(df_sigma['year'], df_sigma['sigma_weighted'], 'r-', linewidth=2, label='Pop-weighted')
...
for cc, (name, color) in list(growth_cases.items())[:8]:
    cdata = mad[(mad['countrycode'] == cc) & (mad['gdppc'].notna()) & (mad['year'] >= 1900)].sort_values('year')
    if len(cdata) > 0:
        ax.plot(cdata['year'], cdata['gdppc'], color=color, linewidth=2, label=name)
ax.set_yscale('log')
...
for _, grp in reg215.groupby('region_code'):
    ...
    if 'World' in rname:
        continue
    data = grp.sort_values('reporting_year')
    ax.plot(data['reporting_year'], data['pop_in_poverty'] / 1e9, '-', linewidth=2, label=rname[:25])
...
for pl in [2.15, 3.65, 6.85]:
    ...
    if gap is not None and len(gap) > 0:
        ...
        summary_text += f"  {int(first_valid['year'])}: {first_valid['gap_pct_gdp']:.2f}% of GDP\n"

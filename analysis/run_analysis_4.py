...
fig.suptitle("The USSR Growth Story: Industrialization Without Markets", fontsize=16, fontweight='bold')
...
for cc in mad['countrycode'].unique():
    if 'USSR' in str(mad[mad['countrycode'] == cc]['country'].iloc[0]) or cc in ussr_codes:
        cdata = mad[(mad['countrycode'] == cc) & (mad['gdppc'].notna())].sort_values('year')
        if len(cdata) > 0:
            print(f"  Found: {cc} = {cdata.iloc[0]['country']}, {len(cdata)} obs, {cdata['year'].min()}-{cdata['year'].max()}")
...
for cc, (name, color, ls) in comparators.items():
    cdata = mad[(mad['countrycode'] == cc) & (mad['gdppc'].notna()) & (mad['year'] >= 1870)].sort_values('year')
    if len(cdata) > 0:
        ax.plot(cdata['year'], cdata['gdppc'], ls, color=color, linewidth=2.5, label=name)
...
decades = [(1920, 1930), (1930, 1940), (1940, 1950), (1950, 1960), (1960, 1970), 
           (1970, 1980), (1980, 1990), (1990, 2000), (2000, 2010), (2010, 2022)]

print(f"\n  Growth rates by decade:")
print(f"  {'Decade':<12} {'Russia/USSR':<14} {'China':<10} {'S. Korea':<10} {'India':<10} {'USA':<10}")

decade_data = {}
for cc, name in [('RUS', 'Russia/USSR'), ('CHN', 'China'), ('KOR', 'S. Korea'), ('IND', 'India'), ('USA', 'USA')]:
    decade_data[name] = []
    for ds, de in decades:
        ...
        if len(start) > 0 and len(end) > 0 and start.iloc[0]['gdppc'] > 0:
            g = ((end.iloc[0]['gdppc'] / start.iloc[0]['gdppc']) ** (1/(de-ds)) - 1) * 100
...
width = 0.18
colors_bar = {'Russia/USSR': '#d62728', 'China': '#e377c2', 'S. Korea': '#8c564b', 'India': '#7f7f7f', 'USA': '#1f77b4'}
for i, (name, vals) in enumerate(decade_data.items()):
    vals_clean = [v if not np.isnan(v) else 0 for v in vals]
    ax.bar(x + i*width, vals_clean, width, label=name, color=colors_bar[name], alpha=0.8)
...
for start_label, rus_start, chn_start, ls in [
    ...
    if len(rus_base) > 0 and len(chn_base) > 0:
        ...
        ax.plot(rus_subset['t'], rus_subset['gdppc_idx'], ls, color='#d62728', linewidth=2.5,
                label=f"Russia from {rus_start}")
        ax.plot(chn_subset['t'], chn_subset['gdppc_idx'], ls, color='#e377c2', linewidth=2.5,
...
for yr in [1913, 1928, 1940, 1950, 1960, 1970, 1980, 1989, 1991, 1998, 2008, 2022]:
    row = rus[rus['year'] == yr]
    if len(row) > 0:
        ratio_to_usa = row.iloc[0]['gdppc'] / mad[(mad['countrycode'] == 'USA') & (mad['year'] == yr)].iloc[0]['gdppc'] * 100 if len(mad[(mad['countrycode'] == 'USA') & (mad['year'] == yr)]) > 0 else 0
...
for cc in mad['countrycode'].unique():
    ...
    for i in range(len(cdata)):
        for j in range(i+1, len(cdata)):
            ...
            if span > 40:
                break
            g = ((cdata.iloc[j]['gdppc'] / cdata.iloc[i]['gdppc']) ** (1/span) - 1) * 100
...
for _, ep in df_ep.head(50).iterrows():
    ...
    is_resource = cc in ['AGO', 'GNQ', 'TKM', 'AZE', 'BHR', 'OMN', 'SAU', 'QAT', 'KWT', 'LBY', 'ARE']
    ...
    print(f"  {ep['country'][:25]:<25} {ep['start']}-{ep['end']:<8} {ep['span']:<6} {ep['growth']:.1f}%   ${ep['gdp_start']:>8,.0f}   {tag} {system[:35]}")

# Also specifically check USSR growth
print(f"\n\n  USSR/RUSSIA DETAILED GROWTH PERIODS:")
rus = mad[(mad['countrycode'] == 'RUS') & (mad['gdppc'].notna())].sort_values('year')
periods = [(1928, 1940, 'Stalin industrialization'), (1940, 1950, 'WWII'),
           (1950, 1960, 'Post-war recovery'), (1960, 1970, 'Mature Soviet'),
           (1970, 1980, 'Stagnation begins'), (1980, 1989, 'Late Soviet/Perestroika'),
           (1989, 1998, 'Collapse'), (1998, 2008, 'Putin oil boom'), (2008, 2022, 'Stagnation')]

for ps, pe, label in periods:
    start = rus[rus['year'] == ps]
    end = rus[rus['year'] == pe]
    if len(start) > 0 and len(end) > 0 and start.iloc[0]['gdppc'] > 0:
        g = ((end.iloc[0]['gdppc'] / start.iloc[0]['gdppc']) ** (1/(pe-ps)) - 1) * 100
        print(f"    {ps}-{pe} ({label}): {g:.1f}%/yr (${start.iloc[0]['gdppc']:,.0f} → ${end.iloc[0]['gdppc']:,.0f})")
...
table.scale(1.1, 1.5)
ax.set_title("Longest Sustained >4% Growth Episodes\n(MKT=market, CMD=command, RES=resource)", fontweight='bold', pad=20)
...
base_year = 1950
for cc, (name, color, ls) in comparators.items():
    cdata = mad[(mad['countrycode'] == cc) & (mad['gdppc'].notna()) & (mad['year'] >= base_year)].sort_values('year')
...
for cc, name, color in [('RUS', 'Russia/USSR', '#d62728'), ('CHN', 'China', '#e377c2'), 
                          ('KOR', 'S. Korea', '#8c564b'), ('JPN', 'Japan', '#9467bd')]:
    cdata = mad[(mad['countrycode'] == cc) & (mad['gdppc'].notna())].sort_values('year')
    rolling_g = []
    ...
    if rolling_g:
        ...
        ax.plot(df_rg['year'], df_rg['growth'], '-', color=color, linewidth=2.5, label=name)

ax.set_title("10-Year Rolling GDP/capita Growth Rate\n(The command economy trajectory: fast start, then stall)")
...
if len(mao_start) > 0 and len(mao_end) > 0 and len(reform_end) > 0:
    g_mao = ((mao_end.iloc[0]['gdppc'] / mao_start.iloc[0]['gdppc']) ** (1/29) - 1) * 100
    g_reform = ((reform_end.iloc[0]['gdppc'] / mao_end.iloc[0]['gdppc']) ** (1/44) - 1) * 100
    print(f"\n  OVERALL:")

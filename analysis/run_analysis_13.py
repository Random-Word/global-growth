...
df = raw.pivot_table(
    index=["cc", "country", "year"], columns="indicator", values="value"
).reset_index()
...
fig.suptitle(
    'The "Greed" Question: Is Foreign Aid Keeping Up?', fontsize=16, fontweight="bold"
)
...
for cc, col in zip(oda_recipients, oda_colors):
    ...
    if len(d) > 0:
        ax.plot(
            d["year"], d["oda_pct_gni_donor"], label=NAMES[cc], color=col, linewidth=1.5
        )
...
dev_ccs_oda = [
    cc
    for r in ["East Asia", "South Asia", "Sub-Saharan Africa", "Latin America"]
    for cc in REGIONS[r]
]
total_oda = (
    df[df["cc"].isin(dev_ccs_oda)]
    .groupby("year")["oda_net_current_usd"]
    .sum()
    .reset_index()
)
...
if for_plot:
    ...
    h = 0.35
    bars1 = ax.barh(
        y - h / 2, fp["remittances"], h, label="Remittances", color="#2ecc71"
    )
    bars2 = ax.barh(
        y + h / 2, fp["oda"], h, label="ODA received", color="#e74c3c", alpha=0.8
    )
    ax.set_yticks(y)
...
for cc in ["ETH", "MOZ", "RWA", "TZA", "BGD", "GHA", "KEN", "IND"]:
    ...
    if len(d) >= 2:
        ...
        if len(early) > 0 and len(late) > 0:
            ...
            print(
                f"  {NAMES[cc]:12s}: {e['oda_pct_gni_donor']:.1f}% ({int(e['year'])}) -> "
...
fig.suptitle(
    "The Development Recipe: What Distinguishes Countries That Took Off?",
    fontsize=16,
    fontweight="bold",
)
...
for cc in dev_ccs:
    ...
    if len(d) >= 2:
        ...
        if len(early) > 0 and len(late) > 0:
            growth = (
                late.iloc[-1]["gdppc_ppp"] / early.iloc[-1]["gdppc_ppp"] - 1
            ) * 100
...
metrics = [
    (
        ...
        "Avg Savings Rate (% GDP)",
        "Savings → Investment → Growth",
    ),
    (
        "gross_fixed_capital_pct_gdp",
        "Avg Fixed Capital Investment (% GDP)",
...
fig, axes = plt.subplots(2, 3, figsize=(20, 12))
fig.suptitle(
    "Divergent Paths: What Made East Asia Different?", fontsize=16, fontweight="bold"
)
...
for region in compare_regions:
    ...
    if len(med) > 0:
        ax.plot(
            med["year"],
            med["gross_fixed_capital_pct_gdp"],
            label=region,
            color=REGION_COLORS[region],
            linewidth=2.5,
        )
        ...
        growth_ranks.append(
            {
                ...
                "start_gdp": first["gdppc_ppp"],
                "end_gdp": last["gdppc_ppp"],
...
fig.suptitle(
    "Development Success Stories: Who Grew Fastest and How?",
    fontsize=16,
    fontweight="bold",
)
...
fig.suptitle(
    "The Demographic Dividend: Fertility Decline and Economic Growth",
    fontsize=16,
    fontweight="bold",
)
        ...
        if len(early) > 0 and len(late) > 0:
            ...
            if len(gdp_d) >= 2:
                ...
                if len(gdp_early) > 0 and len(gdp_late) > 0:
                    growth = (
                        gdp_late.iloc[-1]["gdppc_ppp"] / gdp_early.iloc[-1]["gdppc_ppp"]
                        - 1
                    ) * 100
                    fert_pairs.append(
                        {
                            ...
                            "fert_decline": early.iloc[-1]["fertility_rate"]
                            - late.iloc[-1]["fertility_rate"],
                            "gdp_growth": growth,
                            "region": cc_region.get(cc, ""),
                        }
                    )
...
for region in ["East Asia", "South Asia", "Sub-Saharan Africa", "Latin America"]:
    ...
    ax.scatter(
        fp.loc[mask, "fert_decline"],
        fp.loc[mask, "gdp_growth"],
        c=REGION_COLORS[region],
        label=region,
        s=80,
        alpha=0.8,
...
for _, row in fp.iterrows():
    ax.annotate(
        row["name"],
        (row["fert_decline"], row["gdp_growth"]),
        fontsize=6.5,
        ha="center",
        va="bottom",
    )
...
for cc, color in showcase:
    ...
    if len(d) > 0:
        ax.plot(
            d["year"], d["fertility_rate"], label=NAMES[cc], color=color, linewidth=2
        )
...
fig, axes = plt.subplots(2, 2, figsize=(16, 12))
fig.suptitle(
    "Beyond ODA: The Full Picture of Rich-to-Poor Resource Flows",
    fontsize=16,
    fontweight="bold",
)
...
for cc in ssa_ccs:
    ...
    if len(gdp_d) >= 2:
        ...
        if len(gdp_early) > 0 and len(gdp_late) > 0:
            growth = (
                gdp_late.iloc[-1]["gdppc_ppp"] / gdp_early.iloc[-1]["gdppc_ppp"] - 1
            ) * 100
            # avg ODA received (use oda_net_current_usd as proxy)
            oda_avg = df[
                (df["cc"] == cc)
                & (df["oda_pct_gni_donor"].notna())
                & (df["year"].between(1995, 2015))
            ]["oda_pct_gni_donor"].mean()
            if not np.isnan(oda_avg):
                ax.scatter(
                    oda_avg,
                    growth,
                    c=REGION_COLORS["Sub-Saharan Africa"],
                    s=80,
                    alpha=0.8,
                    edgecolors="white",
                )
...
ax.set_title(
    "Development Recipe: Key Differences", fontsize=12, fontweight="bold", pad=20
)
...
dev_ccs_all = [
    cc
    for r in ["East Asia", "South Asia", "Sub-Saharan Africa", "Latin America", "Other"]
    for cc in REGIONS[r]
]
xctry = []
...
for cc, color in showcase_grad:
    ...
    if len(oda_ts) > 3 and len(fdi_ts) > 3:
        ...
        oda_smooth = (
            oda_ts.set_index("year")["oda_pct_gni_donor"]
            .rolling(5, center=True, min_periods=2)
            .mean()
        )
        fdi_smooth = (
            fdi_ts.set_index("year")["fdi_pct_gdp"]
            .rolling(5, center=True, min_periods=2)
            .mean()
        )
        ax.plot(
...
ax.set_title(
    "ODA Graduation: ODA (solid) Falls,\nFDI (dashed) Rises",
    fontsize=10,
    fontweight="bold",
)
...
p2 = ax.bar(
    x_pos, fdi_vals, width, bottom=oda_vals, label="FDI", color="#3498db", alpha=0.8
)
p3 = ax.bar(
    x_pos,
    rem_vals,
...
for i in range(2):
    ...
    ax.text(
        i,
        oda_vals[i] + fdi_vals[i] + rem_vals[i] / 2,
        f"{rem_vals[i]:.1f}%",
        ha="center",
        va="center",
        fontsize=9,
...
fig, axes = plt.subplots(2, 3, figsize=(20, 12))
fig.suptitle(
    "The Capital Flow Transition: From Aid Dependency to Investment-Led Growth",
    fontsize=16,
    fontweight="bold",
)
...
for cc in ["KOR", "CHN", "VNM", "ETH", "RWA", "BGD", "IDN", "GHA"]:
    ...
    print(
        f"  {NAMES[cc]:12s}: ODA {oda_e:5.1f}→{oda_l:5.1f}  "

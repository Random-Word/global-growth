...
# ── Country groups ──────────────────────────────────────────────────────────
SUCCESS = {
    'KOR': 'South Korea', 'CHN': 'China', 'VNM': 'Vietnam',
    'BGD': 'Bangladesh', 'BWA': 'Botswana', 'THA': 'Thailand',
    'MYS': 'Malaysia', 'IDN': 'Indonesia', 'CHL': 'Chile',
    'RWA': 'Rwanda', 'IND': 'India',
}
CHALLENGE_SSA = {
    'GHA': 'Ghana', 'KEN': 'Kenya', 'SEN': 'Senegal',
    'MOZ': 'Mozambique', 'ZMB': 'Zambia', 'TZA': 'Tanzania',
    'NGA': 'Nigeria', 'ZAF': 'South Africa',
}
LATAM_DEBT = {
    'ARG': 'Argentina', 'BRA': 'Brazil', 'MEX': 'Mexico',
    'COL': 'Colombia', 'PER': 'Peru', 'ECU': 'Ecuador',
}
...
for eco, label in REGIONS.items():
    ...
    if len(sub) > 0:
        ax.plot(sub['year'], sub['value'], label=label, color=colors_reg[eco], linewidth=2)

# Mark key events
ax.axvline(1982, color='gray', linestyle='--', alpha=0.5, linewidth=1)
ax.text(1982.5, ax.get_ylim()[1] * 0.9 if ax.get_ylim()[1] > 0 else 6, 'Latam\ndebt crisis',
        fontsize=8, color='gray', va='top')
ax.axvline(1996, color='gray', linestyle='--', alpha=0.5, linewidth=1)
ax.text(1996.5, ax.get_ylim()[1] * 0.7 if ax.get_ylim()[1] > 0 else 4.5, 'HIPC\nlaunched',
        fontsize=8, color='gray', va='top')
ax.axvline(2005, color='gray', linestyle='--', alpha=0.5, linewidth=1)
ax.text(2005.5, ax.get_ylim()[1] * 0.5 if ax.get_ylim()[1] > 0 else 3, 'MDRI\nrelief',
        fontsize=8, color='gray', va='top')
...
ax.set_title('B. External Debt Stock (% of GNI) by Region', fontsize=13, fontweight='bold')
ax.set_xlabel('Year')
ax.set_ylabel('External debt (% of GNI)')
ax.legend(fontsize=9, loc='upper right')
ax.set_xlim(1970, 2023)

plt.suptitle('Debt Burdens by Region, 1970–2023', fontsize=15, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(CHARTS_DIR, '66_regional_debt_over_time.png'), dpi=150, bbox_inches='tight')
plt.close()
...
ax = axes[0]
for gdf, color, ls in [(success_avg, '#2196F3', '-'),
                        (ssa_avg, '#F44336', '-'),
                        (latam_avg, '#FF9800', '-')]:
    gdf = gdf.sort_values('year')
    ax.plot(gdf['year'], gdf['value'], label=gdf['group'].iloc[0],
...
plt.suptitle('Debt Burdens: Development Successes vs Challenges, 1970–2023',
             fontsize=15, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(CHARTS_DIR, '67_success_vs_challenge_debt.png'), dpi=150, bbox_inches='tight')
plt.close()
...
KEY_SUCCESS = {'KOR': 'S. Korea', 'CHN': 'China', 'VNM': 'Vietnam',
               'BWA': 'Botswana', 'BGD': 'Bangladesh', 'THA': 'Thailand'}
KEY_CHALLENGE = {'ARG': 'Argentina', 'GHA': 'Ghana', 'ZMB': 'Zambia',
                 'MOZ': 'Mozambique', 'KEN': 'Kenya', 'BRA': 'Brazil'}
...
ax.set_title('A. Development Successes — External Debt (% GNI)', fontsize=12, fontweight='bold')
ax.set_xlabel('Year')
ax.set_ylabel('External debt (% of GNI)')
ax.legend(fontsize=9, loc='upper right')
ax.set_xlim(1970, 2023)
...
ax.set_title('B. Development Challenges — External Debt (% GNI)', fontsize=12, fontweight='bold')
ax.set_xlabel('Year')
ax.set_ylabel('External debt (% of GNI)')
ax.legend(fontsize=9, loc='upper right')
ax.set_xlim(1970, 2023)
...
plt.suptitle('Individual Country Debt Trajectories, 1970–2023',
             fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(CHARTS_DIR, '68_individual_debt_trajectories.png'), dpi=150, bbox_inches='tight')
plt.close()
...
latest = ds_rev.groupby('economy').apply(
    lambda x: x.nlargest(1, 'year')
).droplevel(1).reset_index()

latest['name'] = latest['economy'].map(ALL)
latest = latest.dropna(subset=['name'])
latest['group'] = latest['economy'].apply(
    lambda x: 'Success' if x in SUCCESS else ('SSA Challenge' if x in CHALLENGE_SSA else 'Latin America'))

fig, axes = plt.subplots(1, 2, figsize=(16, 7))

# Panel A: Debt service as % of revenue, latest year
ax = axes[0]
group_colors = {'Success': '#2196F3', 'SSA Challenge': '#F44336', 'Latin America': '#FF9800'}
latest_sorted = latest.sort_values('ds_pct_revenue', ascending=True)
bars = ax.barh(latest_sorted['name'], latest_sorted['ds_pct_revenue'],
               color=[group_colors[g] for g in latest_sorted['group']])
ax.axvline(30, color='red', linestyle='--', alpha=0.6, linewidth=1.5)
ax.text(31, len(latest_sorted) - 1, 'Debt distress\nthreshold (~30%)',
        color='red', fontsize=9, va='top')
ax.set_title('A. Debt Service as % of Government Revenue\n(latest available year)',
...
ax = axes[1]
for group_dict, label, color in [(SUCCESS, 'Successes', '#2196F3'),
    ...
    if len(avg) > 0:
        ax.plot(avg['year'], avg['ds_pct_revenue'], label=label,
                color=color, linewidth=2.5)

ax.axhline(30, color='red', linestyle='--', alpha=0.6, linewidth=1.5)
ax.set_title('B. Debt Service as % of Revenue Over Time', fontsize=12, fontweight='bold')
ax.set_xlabel('Year')
ax.set_ylabel('Debt service / Revenue (%)')
ax.legend(fontsize=10)
ax.set_xlim(1990, 2023)

plt.suptitle('The Real Constraint: How Much Revenue Goes to Debt Service?',
             fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(CHARTS_DIR, '69_debt_service_vs_revenue.png'), dpi=150, bbox_inches='tight')
plt.close()
...
try:
    ...
    bilateral = fetch_indicator('DT.DOD.BLAT.CD', {**CHALLENGE_SSA, **LATAM_DEBT, **SUCCESS})
    multilateral = fetch_indicator('DT.DOD.MLAT.CD', {**CHALLENGE_SSA, **LATAM_DEBT, **SUCCESS})
    private_debt = fetch_indicator('DT.DOD.PRVT.CD', {**CHALLENGE_SSA, **LATAM_DEBT, **SUCCESS})
    has_composition = len(bilateral) > 0 and len(multilateral) > 0 and len(private_debt) > 0
except Exception as e:
...
if not has_composition:
    ...
    ssa_comp = pd.DataFrame({
        'Country': ['Ghana', 'Kenya', 'Senegal', 'Zambia', 'Mozambique', 'Tanzania', 'Nigeria', 'S. Africa'],
    ...
    plt.suptitle('Who Holds the Debt? Creditor Composition (~2023, World Bank IDS)',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(CHARTS_DIR, '70_debt_composition.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.suptitle('Who Holds the Debt? Creditor Composition (Latest Available Year)',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(CHARTS_DIR, '70_debt_composition.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
gr_avg = gdp_growth[gdp_growth['year'].between(2000, 2023)].groupby('economy')['value'].mean()
...
for group, color in group_colors.items():
    ...
    sub = scatter_df[mask]
    ax.scatter(sub['debt_service'], sub['gdp_growth'], c=color, s=100,
               label=group, edgecolors='white', linewidth=0.5, zorder=5)
    for _, row in sub.iterrows():
        ax.annotate(row['name'], (row['debt_service'], row['gdp_growth']),
...
ax.text(0.02, 0.02, f'r = {r:.2f}, p = {p:.3f}', transform=ax.transAxes,
...
ax.set_title('Debt Service vs Growth: Development Successes vs Challenges',
             fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.axhline(0, color='gray', linestyle='-', alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(CHARTS_DIR, '71_debt_vs_growth_scatter.png'), dpi=150, bbox_inches='tight')
plt.close()
...
for group_dict, label in [(SUCCESS, 'Successes'), (CHALLENGE_SSA, 'SSA Challenges'), (LATAM_DEBT, 'Latin America')]:

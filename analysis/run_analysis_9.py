...
ax.barh(categories, fossil_vals, color=['#4a4a4a', '#8B4513', '#87CEEB', '#CC3333'], 
...
ax2 = axes[0, 1]
min_cats = ['Copper', 'Manganese', 'Nickel', 'Graphite', 'Rare earths', 'Cobalt', 'Lithium',
            'TOTAL\nMinerals']
min_vals = [22000, 20000, 3600, 1800, 350, 220, 180, transition_total_kt]
colors2 = ['#B87333', '#9370DB', '#C0C0C0', '#2F4F4F', '#DAA520', '#4169E1', '#32CD32', '#CC3333']
...
bars = ax3.barh([1, 0], bar_data, color=['#CC3333', '#32CD32'], height=0.5,
                edgecolor='white')
ax3.set_xscale('log')
ax3.set_xlabel('Million tonnes per year (log scale)')
ax3.set_yticks([0, 1])
ax3.set_yticklabels(bar_labels)
ax3.set_title(f'Mass Ratio: Fossil Fuels are {ratio:.0f}× Heavier', fontweight='bold', fontsize=11)
ax3.set_xlim(1, 100000)

# Add annotation
ax3.annotate(f'We extract {ratio:.0f}× more fossil fuel\nmass than transition minerals.\n'
             f'Even by 2040, transition minerals\nreach ~50 Mt vs fossil ~15,000 Mt.',
             xy=(100, 0.5), fontsize=9, style='italic',
...
categories_waste = ['Fossil fuels\n(fuel + waste)', 'Transition minerals\n(ore + waste)']
fuel_vals_w = [fossil_total_mt, fossil_waste_total]
mineral_vals_w = [transition_total_mt, mineral_waste_total]

x = np.arange(2)
width = 0.35
bars1 = ax4.bar(x - width/2, [fossil_total_mt, transition_total_mt], width, 
                label='Extracted material', color=['#CC3333', '#32CD32'], alpha=0.9)
bars2 = ax4.bar(x + width/2, [fossil_waste_total, mineral_waste_total], width,
                label='Waste rock / overburden', color=['#CC3333', '#32CD32'], alpha=0.4)
ax4.set_yscale('log')
...
# Annotate
ax4.text(0, fossil_total_mt * 1.3, f'{fossil_total_mt:,.0f} Mt', ha='center', fontsize=8, fontweight='bold')
ax4.text(0.35, fossil_waste_total * 1.3, f'{fossil_waste_total:,.0f} Mt', ha='center', fontsize=8)
ax4.text(1, transition_total_mt * 1.5, f'{transition_total_mt:.0f} Mt', ha='center', fontsize=8, fontweight='bold')
ax4.text(1.35, mineral_waste_total * 1.5, f'{mineral_waste_total:.0f} Mt', ha='center', fontsize=8)
ax4.set_ylim(1, 200000)

fig.suptitle('Chart 37: The Scale Gap — Fossil Fuel vs Transition Mineral Extraction',
             fontsize=13, fontweight='bold', y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.96])
plt.savefig(os.path.join(CHART_DIR, '37_extraction_volumes.png'), dpi=150, bbox_inches='tight')
plt.close()
...
fig, axes = plt.subplots(2, 2, figsize=(14, 11), gridspec_kw={'hspace': 0.4, 'wspace': 0.3})
...
mineral_co2_total = (22000*5 + 3600*15 + 180*12 + 220*20 + 1800*3 + 350*30) / 1e6  # Gt
...
for b, v in zip(bars, vals_co2):
    ax.text(b.get_x() + b.get_width()/2, v + 0.5, f'{v:.1f} Gt', ha='center', fontsize=10, fontweight='bold')
ratio_co2 = fossil_co2 / mineral_co2_total
ax.text(0.5, max(vals_co2) * 0.6, f'Fossil fuels emit\n{ratio_co2:.0f}× more CO₂',
...
mineral_water = (22000 * 100 + 3600 * 200 + 90 * 2000 + 90 * 170 + 
                 220 * 150 + 1800 * 50 + 350 * 100) / 1e6  # Gt
categories_water = ['Fossil fuel\nextraction', 'Mineral\nextraction']
vals_water = [fossil_water / 1000, mineral_water]  # both in Gt
bars = ax2.bar(categories_water, vals_water, color=colors_co2, width=0.5, edgecolor='white')
...
for b, v in zip(bars, vals_water):
    ax2.text(b.get_x() + b.get_width()/2, v * 1.05, f'{v:.1f} Gt', ha='center', fontsize=10, fontweight='bold')
ratio_water = vals_water[0] / vals_water[1]
ax2.text(0.5, 0.6, f'Fossil fuels use\n{ratio_water:.0f}× more water',
...
bars = ax3.bar(categories_land, vals_land, color=colors_co2, width=0.5, edgecolor='white')
...
for b, v in zip(bars, vals_land):
    ax3.text(b.get_x() + b.get_width()/2, v * 1.05, f'{v:,.0f} km²', ha='center', fontsize=10, fontweight='bold')
ratio_land = fossil_land / mineral_land
ax3.text(0.5, 0.6, f'Fossil fuels disturb\n{ratio_land:.0f}× more land',
...
bars = ax4.bar(categories_deaths, vals_deaths, color=colors_co2, width=0.5, edgecolor='white')
...
for b, v in zip(bars, vals_deaths):
    ax4.text(b.get_x() + b.get_width()/2, v * 1.1, f'{v:,.0f}k', ha='center', fontsize=10, fontweight='bold')
ratio_deaths = fossil_deaths / mineral_deaths
ax4.text(0.5, 0.6, f'Fossil fuels kill\n{ratio_deaths:.0f}× more people',
        ha='center', fontsize=11, style='italic', transform=ax4.transAxes,
        bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))

fig.suptitle('Chart 38: Ecological Harm — Fossil Fuels vs Transition Minerals',
             fontsize=13, fontweight='bold', y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.96])
plt.savefig(os.path.join(CHART_DIR, '38_harm_comparison.png'), dpi=150, bbox_inches='tight')
plt.close()
...
for i in range(n_years):
    ...
    if i >= 15:
        ...
        mineral_recycled_annual[i] = mineral_virgin_annual[max(0, i-15)] * recycle_rate
...
ax.fill_between(years_cum, 0, fossil_cumul_gt, alpha=0.3, color='#CC3333', label='Fossil fuels (cumulative extraction)')
ax.plot(years_cum, fossil_cumul_gt, color='#CC3333', linewidth=2)
ax.fill_between(years_cum, 0, mineral_total_cumul, alpha=0.3, color='#32CD32', label='Transition minerals (cumulative virgin)')
ax.plot(years_cum, mineral_total_cumul, color='#32CD32', linewidth=2)
ax.set_xlabel('Year')
ax.set_ylabel('Cumulative extraction (Gt)')
ax.set_title('Cumulative Material Extraction\n(NZE Scenario, 2025-2055)', fontweight='bold', fontsize=11)
ax.legend(fontsize=9, loc='upper left')
ax.annotate(f'Even in decline, fossil fuels\nrequire ~{fossil_cumul_gt[-1]:.0f} Gt cumulative extraction\n'
            f'vs ~{mineral_total_cumul[-1]:.1f} Gt for transition minerals',
            xy=(2040, fossil_cumul_gt[15]), xytext=(2035, fossil_cumul_gt[-1] * 0.5),
            fontsize=9, style='italic',
            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))

# Panel 2: Annual virgin material need with recycling
ax2 = axes[1]
ax2.fill_between(years_cum, 0, mineral_virgin_annual, alpha=0.4, color='#FF8C00', label='Virgin mineral extraction')
ax2.fill_between(years_cum, mineral_virgin_annual, mineral_virgin_annual + mineral_recycled_annual,
                 alpha=0.4, color='#32CD32', label='Recycled material')
ax2.plot(years_cum, mineral_virgin_annual + mineral_recycled_annual, color='#2F4F4F', 
         linewidth=2, linestyle='--', label='Total mineral demand')
ax2.plot(years_cum, mineral_virgin_annual, color='#FF8C00', linewidth=2)
ax2.set_xlabel('Year')
ax2.set_ylabel('Million tonnes per year')
ax2.set_title('Transition Mineral Demand\nVirgin vs Recycled (NZE Scenario)', fontweight='bold', fontsize=11)
ax2.legend(fontsize=9, loc='upper left')
ax2.annotate('Recycling of first-generation\nbatteries & solar panels\nkicks in around 2040',
...
fig.suptitle('Chart 39: Burn Once vs Recycle Forever — The Fundamental Asymmetry',
             fontsize=13, fontweight='bold', y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig(os.path.join(CHART_DIR, '39_burn_vs_recycle.png'), dpi=150, bbox_inches='tight')
plt.close()
...
water_users = ['Agriculture\n(Atacama region)', 'Copper mining\n(Chile)', 'Lithium brine\n(Atacama)']
...
ax.set_title('Lithium Water Use in Context\n(Atacama Region, Chile)', fontweight='bold', fontsize=11)
for b, v in zip(bars, water_vals):
    ax.text(v + 0.05, b.get_y() + b.get_height()/2, f'{v:.2f} km³', va='center', fontsize=10)
ax.text(0.5, 0.15, 'Lithium brine uses ~6% of\nregional water — real but small\ncompared to copper & agriculture',
...
ax2_twin = ax2.twinx()
line1, = ax2.plot(years_co, cobalt_per_kwh, 'o-', color='#4169E1', linewidth=2, label='Cobalt per kWh')
line2, = ax2_twin.plot(years_co, lfp_share, 's--', color='#FF8C00', linewidth=2, label='LFP market share (%)')
ax2.set_xlabel('Year')
...
ax2.text(0.05, 0.15, 'Cobalt per kWh: -91% since 2015\n'
         'LFP batteries (zero cobalt)\nnow ~50% of market',
        ha='left', transform=ax2.transAxes, fontsize=9, style='italic',
        bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
...
ax3.bar(years_ni, rest_nickel, bottom=indo_nickel, color='#87CEEB', label='Rest of world', edgecolor='white')
ax3.set_ylabel('Nickel production (Mt)')
ax3.set_title('Indonesia Nickel Boom\n(real deforestation concern)', fontweight='bold', fontsize=11)
ax3.legend(fontsize=9)
ax3.text(0.05, 0.82, 'Indonesian nickel smelting uses\ncoal power + clears tropical forest.\n'
         'This IS a real ecological cost\nof the transition.',
        ha='left', transform=ax3.transAxes, fontsize=9, style='italic',
...
"""
ax4.text(0.02, 0.98, text, fontsize=8.3, fontfamily='monospace', va='top',
         transform=ax4.transAxes,
         bbox=dict(boxstyle='round', facecolor='#F5F5F5', edgecolor='#333333', alpha=0.9))

fig.suptitle('Chart 40: Real Concerns with Transition Minerals — In Context',
             fontsize=13, fontweight='bold', y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.96])
plt.savefig(os.path.join(CHART_DIR, '40_mineral_concerns.png'), dpi=150, bbox_inches='tight')
plt.close()

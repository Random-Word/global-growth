...
7. **Abundant cheap clean energy is the master key — but not the only lock.** Solar generation grew from 4 TWh (2005) to 2,128 TWh (2024), doubling every ~3.2 years. This S-curve, if it continues, unlocks cascading solutions: electrification eliminates the carbon boundary, cheap desalination addresses freshwater, cheap energy makes recycling and circular economy economically dominant, and cheap energy powers the precision agriculture and controlled-environment farming that could address the nitrogen and land-use boundaries. Crucially, solar is not the only path to energy abundance. Nuclear fission (including small modular reactors from NuScale, Rolls-Royce, X-energy), enhanced geothermal (Fervo Energy's 2023 demonstration at 3.5 MW, Quaise's deep drilling), and eventually fusion (Commonwealth Fusion Systems targeting 2030s) each provide independent routes to cheap clean baseload power. If solar+batteries hit scaling limits — intermittency, mineral supply chains, grid integration — these alternatives mean the energy abundance thesis does not depend on a single technology S-curve. Energy abundance does not automatically solve the biogeochemical boundaries — those also require bioengineering breakthroughs and governance — but it is the necessary enabling condition for nearly every other solution.
...
**But solar is not the only bet.** The energy abundance thesis is de-risked by at least three independent pathways that do not depend on the solar S-curve continuing:

- **Nuclear fission** provides firm baseload power without intermittency or storage requirements. Existing reactors supply ~10% of global electricity. Small modular reactors (NuScale, Rolls-Royce SMR, X-energy) promise lower capital costs and faster deployment than traditional gigawatt plants, though none have reached commercial operation yet. South Korea and France built their grids substantially on fission — demonstrating that nuclear-heavy decarbonization works at national scale.
- **Enhanced geothermal** taps heat from deep hot rock anywhere, not just volcanic regions. Fervo Energy's 2023 Nevada demonstration proved commercial-scale power from non-volcanic geology. Quaise Energy is developing millimeter-wave drilling to reach supercritical temperatures (>400°C) at 10–20 km depth, which would make geothermal available essentially everywhere. Like nuclear, geothermal provides firm baseload — no batteries needed.
- **Fusion** remains speculative but is closer than at any point in history. Commonwealth Fusion Systems (MIT spinout) demonstrated a 20-tesla high-temperature superconducting magnet in 2021 and targets a net-energy demonstration (SPARC) in the late 2020s. TAE Technologies and other private ventures are on parallel tracks. Even if fusion arrives too late for the 2030s energy transition, it could provide effectively unlimited energy for the second half of the century — powering direct air capture, desalination, and industrial recycling at scales that make current projections conservative.

The key insight: if any *one* of solar, nuclear, or geothermal scales to dominance, energy abundance arrives. The thesis does not require all three to succeed — it requires that at least one does. This substantially improves the odds compared to betting on a single S-curve.

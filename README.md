...
5. **The development evidence supports investment as a correlate, not a universal recipe.** In a panel of 222 non-overlapping country spells that excludes World Bank regional aggregates and clusters uncertainty by country, a one-standard-deviation increase in investment is associated with +0.52 percentage points of annualized GDP/capita growth over the ten-year spell (95% CI +0.17 to +0.87). Predictors average its first five years, so exposure and outcome periods overlap. Fertility's coefficient is −0.44 points but its interval crosses zero (−0.95 to +0.06); trade, FDI, schooling, and tax revenue are also insignificant. A separate fixed-window event design finds 43.6% success after high-investment windows, versus 34.9% after medium and 33.3% after low investment, with overlapping confidence intervals. These data do not identify a causal recipe.
...
| **Development recipe** | The 222-spell panel excludes aggregates, uses non-overlapping spells, and clusters uncertainty by country. Investment is positively associated with full-spell growth (+0.52pp per SD, 95% CI +0.17 to +0.87); fertility is not significant at 5% (−0.44pp, CI −0.95 to +0.06). A separate fixed-window comparison finds success rates of 43.6% high, 34.9% medium, and 33.3% low. Restricting to events with a PPP income classification gives 57.7%, 49.1%, and 37.5%, respectively. Intervals overlap in both samples. | First-five-year predictors overlap the ten-year growth outcome; reverse causality, omitted institutions, post-treatment controls, country selection, and the definition of “success” remain. This is descriptive evidence, not proof of a recipe or of necessity. |
...
| **Fragility and causal development** | Conflict-affected, low-capacity, and stable developing spells average 2.3%, 3.1%, and 3.0% subsequent growth; adjusted contrasts are indistinguishable from zero. Matched investment-onset estimates are positive, but joint zero pre-period growth-gap balance is rejected (p=0.019). Only two synthetic cases retain clean donor windows, below the five-case reporting minimum; aggregate effects are withheld. | The balance diagnostic is not a parallel-trends test. Historical WGI missingness, endogenous onset, sparse cases, and no valid assignment rule prevent causal identification. |
...
**Baseline and approximation:** 2025 is the scenario's start label, not a common observation year. The 163 PIP distributions span 2010–2025; distributions from 2019 or earlier represent 8.43% of covered population. They are not nowcast to 2025, population is held fixed, and tax rates span 2008–2024. Each equal-population decile is represented by its mean: the initial net annual gap is approximately \$2.816T and headcount 2.832B, not distribution-integrated PIP estimates. Ignoring within-decile variation understates the monetary gap; headcount error can go either way. Modelled closure is closure of these decile-mean gaps, not proof of eliminating individual poverty. A verified cached comparison covers 106 countries (51.91% of baseline population): the approximate gap is \$0.800T versus PIP's \$0.836T, and headcount 0.771B versus 0.786B. This diagnostic does not recalibrate the scenarios. See [baseline and reconciliation methods](analysis/run_analysis_29.py).
...
- **High investment rates** (25–40% of GDP) — the 222-spell panel finds +0.52 percentage points of annualized full-spell GDP/capita growth per standardized increase (95% CI +0.17 to +0.87). First-five-year predictors overlap the ten-year outcome, so this is not a strictly subsequent-growth regression; reverse causality and omitted institutions remain. In a separate fixed-window event comparison, 43.6% of high-investment events meet the convergence criterion versus 34.9% of medium- and 33.3% of low-investment events; the Wilson intervals overlap. Investment is a measured correlate, not a demonstrated sufficient or necessary cause.
...
Separating conflict-affected, low-capacity non-conflict, and stable developing windows yields subsequent growth averages of 2.3%, 3.1%, and 3.0% per year, respectively, but adjusted contrasts are imprecise and indistinguishable from zero. That null does not show conflict is harmless: conflict selection, survival, reconstruction rebounds, measurement error, and only 17 low-capacity spells make the comparison weak. Fourteen otherwise eligible 1996 windows are excluded for insufficient WGI capacity observations, an explicit early-period availability limitation. A separate investment-onset design produces positive matched estimates but rejects joint zero pre-period growth-gap balance (p=0.019). This tests pre-period levels, not parallel trends; neither rejection nor non-rejection identifies a causal effect. Matching uses exact year and six observed covariates, not region; three of six controls are cross-region. Synthetic donors are same-region and must have complete investment observations at or below 25% of GDP throughout years −5 through +5. Enforcing that rule leaves only Niger and Sierra Leone, below the five-case reporting minimum: aggregate synthetic estimates and intervals are withheld, and [Chart 130](charts/130_causal_development_synthetic_control.png) displays an insufficient-support notice. The two case trajectories remain diagnostic only.
...
**Inference sensitivity:** the central intervals above use country clustering, not two-way clustering. A separate country/year two-way sandwich with small-sample corrections and a Student-t reference gives impact-year revenue **+0.124 [0.039, 0.208]**, GDP **−0.026 [−0.149, 0.097]**, and investment **−0.099 [−0.210, 0.011]**. Two-way revenue intervals include zero at horizons 3–5. There are only 30–34 year clusters, which are not independent price-innovation counts; neither covariance specification resolves identification. [Analysis 33](analysis/run_analysis_33.py) records both methods and the Pink Sheet source URL, cached-file hash and byte count; the acquisition date/release remains unverified. It runs offline after Analyses 29/30 and requires the official Pink Sheet annual workbook and WDI export inputs to be cached.
...
*The post-onset association is positive, but joint zero pre-period growth-gap balance is rejected (p=0.019). This is not a parallel-trends test; endogenous onset and unobserved shocks prevent causal identification.*
...
**Publication validation (September 2026):** 90 regression tests pass; supporting Analyses 29, 30 and 33 were rerun from caches. [analysis/validate_outputs.py](analysis/validate_outputs.py) checks generated schemas and, with `--publication`, requires linked source/charts in the Git index rather than merely on disk. This is not certification of every historical calculation. The full Analysis 18 run currently stops in its legacy poverty-cost sensitivity because compatible country-gap observations are absent from the available cache; it now reports an actionable error, not a substituted conversion. Its `--development-only` option independently reproduces Chart 96 and its 222-spell results. Legacy mixed-basis Charts 00/06 are withdrawal notices; Charts 01/01b explicitly remain invalid diagnostics superseded by Chart 118.
